...
* **Data Residency and Compliance**: Keep sensitive data within your infrastructure and ensure that data remains in the location where the resource owner resides, which helps you comply with data residency regulations.
* **Reduced Latency**: Host the gateway within your own infrastructure to process API requests closer to your services, which minimizes latency and enhances performance.
...
* **Scalability and Flexibility**: Control the scaling of the gateway.
...
Your hybrid gateway connects to the Cloud Control Plane through API endpoints exposed by Gravitee's secured Cloud Gate. These endpoints ensure that your gateways stay up-to-date with your configuration and report analytics back to your cloud environment so you have one unified view of analytics in the Gravitee Cloud Control Plane.

Authentication and authorization to the Cloud Gate is secured by using your Cloud account scoped, signed Cloud Tokens (JWT).
...
Analytics are reported to a Cloud Account dedicated pipeline where Cloud Gate events are produced to Kafka topics, ingested in Logstash, and finally stored in a dedicated Elasticsearch index that your Cloud Account's API Management Control Plane consumes.
...
The gateway interacts with two key endpoints:
...
2. **Copy your Cloud license**: To start up and read your APIs, the gateway needs a license. Copy the license and mount it on the hybrid gateway. You get this license through the hybrid gateway setup.
3. **Start up the gateway**: When the gateway starts, it reads the Cloud Token and connects to the targeted Cloud Gate. You are now set to deploy APIs to the gateway.

### Hybrid Gateway wizard

This section explains how to configure a Hybrid Gateway and connect it to your Gravitee Cloud API Management Control Plane environment.
...
1.  On your Gravitee Cloud Dashboard, navigate to **Gateways**, and then click **Deploy Gateway**.
    ...
    <figure><img src="broken-reference" alt=""><figcaption><p>Gravitee Cloud Dashboard with no Gateways deployed.</p></figcaption></figure>

2.  In the **Choose Gateway Deployment Method** pop-up window, select **Hybrid Gateway**.

    <figure><img src="broken-reference" alt=""><figcaption><p>Gravitee Cloud Gateway deployment selection with both Gravitee Hosted Gateways (full SaaS) and Hybrid Gateways as options.</p></figcaption></figure>
...
4.  Select the Gravitee Cloud API Management Environment that you want to connect the Hybrid gateway to.

    <figure><img src="broken-reference" alt=""><figcaption><p>Gravitee Cloud Hybrid Gateway set up guide with selection of platform and environment.</p></figcaption></figure>

5.  In the **Access Point** field, enter the name of your host or hosts that your Hybrid gateway is accessible through. You configured this host in your load balancer or ingress where you run the gateway.

    In Gravitee Cloud, the full resolved URL based on your gateway host is referred to as an Access Point.

    By default, all URLs enforce HTTPS.

6.  To retrieve your Cloud Token and License key, click **Generate Installation Details**.

    {% hint style="info" %}
    For testing purposes, you can also specify `localhost:8082` (to access the Gateway locally).
    {% endhint %}

    <figure><img src="broken-reference" alt=""><figcaption><p>The Deploy Hybrid Gateway screen where you should enter the gateway host that your hybrid gateway will listen to.</p></figcaption></figure>
...
8.  Copy your License, and then add it to your gateway deployment configuration (as described in step 9).
    ...
    <figure><img src="broken-reference" alt=""><figcaption><p>Gravitee Cloud Hybrid Gateway set up with last step where you are able to copy your generated Cloud Token and your License.</p></figcaption></figure>
...
9.  Install the Gravitee APIM Gateway. To install the APIM Gateway, complete any of the following sub-steps:
    ...
    2.  Run the following script:
        ...
        * Replace `<CONTROL_PLANE_VERSION>` with the current version of the Control Plane in Gravitee Cloud.

    {% hint style="info" %}
    **Rate Limiting**: If you want to enable rate-limiting of APIs, consult this documentation to configure your Hybrid Gateway to use a rate-limiting repository, such as Redis: [https://documentation.gravitee.io/apim/configure-apim/apim-components/gravitee-gateway#configure-the-rate-limit-repository](https://documentation.gravitee.io/apim/configure-apim/apim-components/gravitee-gateway#configure-the-rate-limit-repository)
    ...
    * [Helm v3](https://helm.sh/docs/intro/install/)
    ...
    2.  Configure `values.yaml` file.
        ...
        * Replace `<CONTROL_PLANE_VERSION>` with the current version of the Control Plane in Gravitee Cloud (e.g., "4.5.4").
...
10. Click **Return to Overview**. In the **Gateways** section of the **Overview** page, you can see your configured gateway.
    ...
    <figure><img src="broken-reference" alt=""><figcaption><p>Gravitee Cloud Dashboard, now with one hybrid gateway configuration added to Development environment.</p></figcaption></figure>

## Verification

To verify that the gateway is running, make a GET request on the URL you published the gateway on to confirm it is up and running.
