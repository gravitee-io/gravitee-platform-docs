...
The first is to set the `strip_matching_host_port` property to `true`. However, this only works if the port in the header matches the port that Envoy listens on (8443 by default). In the default installation of Ambassador Edge Stack, the public port is 443, which then maps internally to 8443, so this only works for custom installations where the public service port matches the port in the Listener resource. See [the-module-resource.md](../../technical-reference/using-custom-resources/the-module-resource.md "mention") for more information.
...
Refer to the [TLS document](../../technical-reference/tls-configuration/tls-origination.md) for more information on TLS origination.
...
Some [Kubernetes ingress controllers](https://kubernetes.io/docs/concepts/services-networking/ingress/) do not support HTTP/2 fully. As a result, if you are running Ambassador Edge Stack with an ingress controller in front, you may find that gRPC requests fail even with correct Ambassador Edge Stack configuration.

A simple way around this is to use Ambassador Edge Stack with a `LoadBalancer` service, rather than an Ingress controller. You can also consider using Ambassador Edge Stack as your Ingress Controller. See [ingress-controller.md](../../technical-reference/ingress-and-load-balancing/ingress-controller.md "mention") for more information.
...
Ambassador Edge Stack also supports the gRPC-Web protocol for browser-based gRPC applications. See [#grpc](../../technical-reference/using-custom-resources/the-module-resource.md#grpc "mention") for more information.
