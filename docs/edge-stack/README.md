...
1.  Apply Kubernetes Custom Resource Definitions (CRDs) and wait for the deployment.
    ...
    ```shell
    kubectl apply -f https://app.getambassador.io/yaml/edge-stack/3.13.0/aes-crds.yaml && \
    kubectl wait --timeout=90s --for=condition=available deployment emissary-apiext -n emissary-system
    ```
2.  Install the components and wait for Ambassador Edge Stack.

    ```shell
    kubectl apply -f https://app.getambassador.io/yaml/edge-stack/3.13.0/aes.yaml && \
