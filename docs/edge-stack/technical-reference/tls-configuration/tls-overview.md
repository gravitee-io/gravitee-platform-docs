...
**If no `Host`s are present**, Ambassador Edge Stack synthesizes a `Host` that terminates TLS using a self-signed TLS certificate, and redirects cleartext traffic to HTTPS. You will need to explictly define `Host`s to change this behavior (for example, to use a different certificate or to route cleartext).
...
{% hint style="warning" %}
This implicit `TLSContext` linkage is deprecated and will be removed in a future version of Ambassador Edge Stack; it is not recommended for new configurations. Any other TLS configuration in the `Host` will override this implict `TLSContext` link.
{% endhint %}
