...
Head to [this guide](docs/gko/4.4/guides/define-an-apim-service-account-for-gko.md) to learn how to create a dedicated service account and token for GKO.
...
The example below uses a `bearerToken` to authenticate the requests. Note that the token must have been generated for the account beforehand, as described [here](docs/gko/4.4/guides/define-an-apim-service-account-for-gko.md):
...
* The `ManagementContext` CRD API reference is documented [here](docs/gko/4.4/reference/api-reference.md).
