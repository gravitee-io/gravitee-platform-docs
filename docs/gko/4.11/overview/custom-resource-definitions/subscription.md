...
For mTLS subscriptions, the gateway uses SHA-256 fingerprints to match incoming client certificates to subscriptions. The subscription cache key format is `{apiId}.{planId}.{sha256(clientCertificate)}`. When an application uses a PKCS7 certificate bundle, each certificate in the bundle generates a separate cache entry, allowing any certificate in the bundle to authenticate the subscription.
