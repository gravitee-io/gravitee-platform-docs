---
description: Run the Gravitee Kubernetes Operator 4.11 across a whole cluster or scoped to one namespace, set by a Helm parameter. Compare the two before you install.
---
...
<figure><img src="../../.gitbook/assets/k8s cluster mode.png" alt="One GKO instance watches API definitions in every namespace and creates a ConfigMap in each one."><figcaption><p>Default Cluster Mode architecture</p></figcaption></figure>
{% endtab %}
...
<figure><img src="../../.gitbook/assets/k8s namespaced mode.png" alt="A separate GKO in each namespace creates ConfigMaps that only the local Gateway reads."><figcaption><p>Multiple operators, each listening to its own namespace</p></figcaption></figure>
{% endtab %}
{% endtabs %}
