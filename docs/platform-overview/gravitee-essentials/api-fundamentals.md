...
The structure of REST APIs dictates that each resource is identified by a single URL and the actions on that resource are managed by HTTP verbs (GET, POST, etc.), the request headers, and the request body. This structure forms the basis of the required uniform interface. For each client request, the server makes any requested modifications, then transfers a representation of the state of the requested resource back to the client, hence the designation "representational state transfer." Statelessness ensures that each request contains all of the information necessary for it to be processed by the server, rather than relying on a session state stored by the server.
