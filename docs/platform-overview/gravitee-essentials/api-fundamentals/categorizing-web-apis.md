...
For use cases that focus on real-time applications, event-driven architecture and asynchronous APIs enable communication that is significantly more efficient. EDA is a completely different way to think about structuring an application’s architecture which can cause a number of integration challenges, both internally and externally. The decision to use asynchronous APIs is context-dependent. It is quite common for a single system to leverage both synchronous and asynchronous APIs, and asynchronous APIs effectuate significant benefits when applied in suitable contexts.&#x20;
