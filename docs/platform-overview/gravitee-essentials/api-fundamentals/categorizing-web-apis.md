...
In summary, the shift from monoliths to microservices decouples application components and interfacing via web APIs, which were initially synchronous and adhered tightly to the request-response network communication model. Although these components communicated synchronously, the execution flow of applications did not have to be synchronous. As the single-threaded language that powers the web, Javascript is inherently asynchronous because the main execution thread cannot be blocked by synchronous web API communication. Asynchronous communication that decouples information producers from information consumers enables powerful functionality. However, this requires a restructuring of application logic from the backend to the UI to create a system with an entirely different architectural style referred to as event-driven architecture.
