...
To frame a relatable analogy, consider that humans [cannot multitask](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7075496/) when performing high-level functions, i.e., a person can sustain only a single train of thought. This is conducive to humans planning a synchronous execution of tasks, where task A must be completed before moving on to task B. For example, someone may plan to get a quote from an insurance provider, then write a report.
...
The programming equivalent of a single train of thought is a single thread of execution. In each case, the linear and sequential execution flow of tasks can be broken up and executed concurrently through the involvement of a third party. In the real-world asynchronous example, this third party is the insurance provider. In asynchronous programming, the third party is generally a web API called by the main execution thread.
