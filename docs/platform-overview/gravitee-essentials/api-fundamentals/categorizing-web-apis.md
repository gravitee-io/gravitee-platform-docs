...
The decision to use asynchronous APIs is context-dependent. It is quite common for a single system to leverage both synchronous and asynchronous APIs, and asynchronous APIs effectuate significant benefits when applied in suitable contexts. Asynchronous API clients do not initiate communication beyond expressing initial interest in a data stream. Instead, communication is triggered by events, which are changes in state. The pattern reduces to a client subscribing to a particular data stream, a change of state occurring, a broker delivering this change of state to all subscribed clients, and each client processing this data for its particular end use. Entire systems built around this asynchronous communication style employ what is broadly known as **event-driven architecture** (EDA). This architectural style exists at the system level instead of the API level.
