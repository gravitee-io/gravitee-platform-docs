...
However, humans often adopt asynchronous task management, e.g., begin task A, switch to task B, then return to task A, etc. For example, if the insurance provider places the potential customer on hold, the caller can switch to working on their report, then switch back to the phone conversation once they are connected with an agent.

A person's single train of thought is equivalent to a single execution thread in the context of programming, and both support asynchronous execution, in which the linear and sequential execution flow of tasks can be interrupted through the involvement of a third party. In the real-world example, this third party is the insurance provider, but in programming, the third party is generally a web API called by the main execution thread.
