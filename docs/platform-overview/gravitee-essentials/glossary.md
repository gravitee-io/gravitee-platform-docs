...
* **Gravitee Sharding Tag:** A tag used to deploy a gateway API to a subset of gateways. A sharding tag can be mapped to a gateway’s fully qualified domain name to prompt the developer portal to display the access URL of the API with the identical sharding tag.
