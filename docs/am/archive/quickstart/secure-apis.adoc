= Secure APIs with Gravitee API Management
...
We assume that you have link:{{ '/apim/3.x/apim_installguide.html' | relative_url }}[installed Gravitee API Management^] and have a fully operational environment which can interact with your published APIs (see link:{{ '/apim/3.x/apim_quickstart_publish.html' | relative_url }}[Publish your first API^]).
