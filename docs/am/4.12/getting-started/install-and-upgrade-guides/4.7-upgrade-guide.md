...
metaLinks:
  alternates:
    - >-
      https://app.gitbook.com/s/H4VhZJXn1S232OEmh8Wv/getting-started/install-and-upgrade-guides/4.7-upgrade-guide
---
...
Access Management 4.12 requires the following database migrations:

- `4.12.0-applications-add-sub-type`: Adds a `sub_type` column to the `applications` table to support agent application personas (User Embedded, Hosted Delegated, Autonomous).
- `4.12.0-trust-domains-table`: Creates the `trust_domains` table to store SPIFFE trust domain configurations for workload identity validation.

These migrations are applied automatically on startup.

### Breaking Changes

**JWT-Bearer Assertion Semantics**: Standard `jwt-bearer` client assertions (`client_assertion_type=urn:ietf:params:oauth:client-assertion-type:jwt-bearer`) now enforce strict RFC 7523 semantics. The `iss` claim must equal the `sub` claim, and both must equal the `client_id`. Assertions with a SPIFFE `sub` are rejected with error: "SPIFFE JWT-SVID must be sent with client_assertion_type=jwt-spiffe".

**Application Model**: The application model now uses a top-level `subType` field instead of the nested `settings.agent` structure. Existing applications are migrated automatically, but custom integrations reading or writing the `settings.agent` field must be updated to use `subType`.
