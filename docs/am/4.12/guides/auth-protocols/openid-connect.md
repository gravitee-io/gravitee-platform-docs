...
When CIMD is enabled for a domain, the OIDC discovery document includes `"client_id_metadata_document_supported": true`. This indicates that clients can present a URL as their `client_id` and retrieve configuration dynamically from that URL.
