...
{% hint style="info" %}
We advise you to read [get user profile information](docs/am/4.5/getting-started/tutorial-getting-started-with-am/get-user-profile-information.md) first.
{% endhint %}
...
{% hint style="info" %}
When the roles are updated via SCIM, the roles already applied via Role Mappers won’t be persisted as an assigned role. This ensures that it can be safely removed when the mapper rule does not match anymore. For more about SCIM, click [here](docs/am/4.5/guides/auth-protocols/scim-2.0.md).
{% endhint %}
