...
When using the implicit flow, all tokens are returned from the [authorization endpoint](oauth-2.0/#authorization-endpoint). The token endpoint is not used.
...
When using the Hybrid flow, some tokens are returned from the [authorization endpoint](oauth-2.0/#authorization-endpoint) and others are returned from the [token endpoint](oauth-2.0/#token-endpoint). Hybrid is used by clients who want tokens separately from front channel and back channel.
