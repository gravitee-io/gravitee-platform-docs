...
* **Enroll MFA:** Enroll MFA factors are based on user profile information and skip the interactive MFA enrollment step. [Functions that use Gravitee Expression Language](docs/am/4.9/guides/am-expression-language.md#functions) can pull information from user profiles. MFA flows let you use multiple MFA for authentication.
...
* **MFA Challenge (EE):** Enforces users to confirm their identity by using another factor. [Functions that use Gravitee Expression Language](docs/am/4.9/guides/am-expression-language.md#functions) can pull information from user profiles. MFA flows let you use multiple MFA for authentication.
