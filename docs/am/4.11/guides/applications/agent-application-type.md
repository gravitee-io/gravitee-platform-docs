...
Agent applications are OAuth clients with restricted grant types to align with agentic use cases. They default to confidential client type and support only `authorization_code` and `client_credentials` flows. The `implicit`, `password`, and `refresh_token` grants are forbidden. Agent applications appear in a dedicated "Agents" section in the UI and are tracked separately in audit logs.
...
1. Navigate to the "Agents" section in the Access Management console and select "Create Agent."
