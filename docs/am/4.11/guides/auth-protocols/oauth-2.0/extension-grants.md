...
Token exchange enables MCP Servers and Protected Resources to exchange subject tokens for new access tokens. This functionality is configured at the domain level using the `urn:ietf:params:oauth:grant-type:token-exchange` grant type.

### Configuration properties

Configure token exchange settings at the domain level to control which token types can be exchanged.

| Property | Type | Description |
|:---------|:-----|:------------|
| `tokenExchangeSettings.enabled` | Boolean | Enables the token exchange grant type (`urn:ietf:params:oauth:grant-type:token-exchange`) for the domain |
| `tokenExchangeSettings.allowedSubjectTokenTypes` | Array of strings | Permitted subject token type URNs for exchange requests. Valid values include `urn:ietf:params:oauth:token-type:access_token`, `urn:ietf:params:oauth:token-type:refresh_token`, and `urn:ietf:params:oauth:token-type:id_token` |
...
When `tokenExchangeSettings.enabled` is set to `true`, the domain accepts token exchange requests using the `urn:ietf:params:oauth:grant-type:token-exchange` grant type. The `allowedSubjectTokenTypes` array restricts which token types can be submitted as subject tokens in exchange requests.

During token exchange:

1. The system validates that the subject token type matches one of the URNs in `allowedSubjectTokenTypes`.
2. If the subject token type is not in the allowed list, the request fails with an `invalid_request` error.
3. The system verifies the subject token's signature and expiration.
4. A new access token is issued with a lifetime that cannot exceed the subject token's remaining validity.

Token exchange requests don't issue refresh tokens or ID tokens, even if the `openid` scope is requested.