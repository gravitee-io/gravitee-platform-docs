...
* [Auth Protocols](guides/auth-protocols/README.md)
  ...
  * [Impact: Add five new endpoints under /organizations/{orgId}/environments/{envId}/domains/{domain}/protected-resources/{resourceId}/secrets: POST (create), POST /{secretId}/_renew (renew), DELETE /{secretId} (delete), GET (list), with request/response schemas for ClientSecret object.](guides/auth-protocols/uma-2.0/protected-resource-secrets.md)
...
* [MCP Servers](guides/mcp-servers/README.md)
  ...
  * [Impact: Document MCP Server (Protected Resource) token exchange workflow: grant_type=urn:ietf:params:oauth:grant-type:token-exchange, subject token validation against allowedSubjectTokenTypes, gis claim extraction, new token issuance with MCP Server clientId as client and audience, lifetime constraint (cannot exceed subject token remaining validity), no refresh/ID tokens issued.](guides/mcp-servers/token-exchange.md)
  * [Create an MCP Server](guides/mcp-servers/create-an-mcp-server.md)
  * [MCP Tools](guides/mcp-servers/mcp-tools.md)
  * [Protected Resource Secret Management Overview + Managing Protected Resource Secrets + Protected Resource Configuration  + Impact: Document that UI filters token endpoint authentication methods in MCP Server context, showing only client_secret_basic, client_secret_post, and client_secret_jwt. Methods like private_key_jwt, tls_client_auth, and none are hidden.](guides/mcp-servers/secret-management.md)
