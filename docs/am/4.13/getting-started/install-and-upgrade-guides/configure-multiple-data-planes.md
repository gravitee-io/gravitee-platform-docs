...
A data plane can also be provisioned at runtime instead of being declared in the **gravitee.yaml**. Post the definition to `/_node/dataplanes` on the technical API of the Management API, with Basic authentication and the same `id`, `name`, `type`, and connection settings as a `dataPlanes` entry. The `id` can't be `default` and can't reuse an identifier already declared in the **gravitee.yaml**. A provisioned data plane is usable immediately, without restarting the node, and is offered as a choice when a user creates a domain in the console. Use `GET /_node/dataplanes` to list the provisioned data planes and `DELETE /_node/dataplanes/{id}` to remove one, once every domain bound to it has been deleted.
