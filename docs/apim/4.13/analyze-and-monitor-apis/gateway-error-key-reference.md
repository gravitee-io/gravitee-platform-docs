---
description: The Gateway records an error key on every failed API Management 4.13 request. Browse the reference to interpret the key you see.
---
...
### On `GATEWAY_CLIENT_CONNECTION_CLOSED`, `GATEWAY_CLIENT_CONNECTION_RESET`, and `GATEWAY_CLIENT_STREAM_ENDED_EARLY`

These keys are misread more often than any other key. None of them means that the API consumer went away, and none means that the Gateway timed out. All three mean the connection closed or reset while the response was still incomplete in HTTP terms.

`GATEWAY_CLIENT_CONNECTION_CLOSED` and `GATEWAY_CLIENT_CONNECTION_RESET` are retried under the same conditions, because one teardown surfaces as a clean close or as a reset depending on the operating system and on timing. They still aren't interchangeable as a diagnosis: a reset also covers an HTTP/2 `RST_STREAM` from the backend, and a connection the backend resets on first use — neither of which involves connection reuse. `GATEWAY_CLIENT_STREAM_ENDED_EARLY` differs in **how far the response had progressed**:
...
* `GATEWAY_CLIENT_CONNECTION_CLOSED` / `GATEWAY_CLIENT_CONNECTION_RESET`. Nothing had reached the API consumer yet, so the Gateway answers `502`.

**From 4.12.18**, a request that qualifies for the retry — body-less, on an idempotent method, received over HTTP/1.1, against the HTTP proxy endpoint of a v4 API — was already retried once, on a fresh connection, before either of these two keys was reported. Seeing `GATEWAY_CLIENT_CONNECTION_CLOSED` or `GATEWAY_CLIENT_CONNECTION_RESET` therefore means either that the request wasn't eligible — a `POST` or a `PATCH`, a request sent with a body, a consumer on HTTP/2, a v2-definition API, a gRPC or WebSocket endpoint — or that the retried attempt failed the same way. Expect the volume of both keys to drop after upgrading, in proportion to the share of traffic that qualifies for the retry. See [Retry on a closed or reset connection](../prepare-a-production-environment/timeout-management.md#retry-on-a-closed-or-reset-connection) for the retry's exact conditions.
