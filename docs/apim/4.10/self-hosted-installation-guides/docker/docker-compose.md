---
description: Install API Management 4.10 with Docker Compose, the quickest self-hosted route. Follow the steps and see what the deployment includes.
metaLinks:
  alternates:
    - docker-compose.md
---
...
{% hint style="warning" %}
The default `graviteeio/apim-gateway` image is based on Alpine Linux, which does not support the ONNX Runtime. As a result, AI policies that depend on ONNX Runtime, such as **AI - Prompt Guard Rails**, do not work with the default image. To use these policies, use the Debian variant of the image instead. For example, `graviteeio/apim-gateway:4.10-debian`. The Debian variant is available for version 4.9 and later.
{% endhint %}
