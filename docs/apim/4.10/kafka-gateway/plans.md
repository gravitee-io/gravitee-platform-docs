...
Mutual TLS (mTLS) for native Kafka APIs provides strong authentication of Kafka clients and improved security for Kafka traffic. mTLS is an additional security layer on top of the TLS already required for native Kafka APIs. With mTLS enabled, both the Kafka Gateway and the Kafka client must present valid certificates to establish a connection. The Kafka client must prove its identity using a client certificate, allowing the Gateway to verify client identities.

### Before: TLS only

- The Kafka client verifies the Gateway's identity using the Gateway's certificate.
- The Gateway doesn't verify the client's identity.
...
For mTLS to work correctly, both the Kafka Gateway and Kafka clients must be configured with the appropriate keystores and truststores. For complete Gateway and client configuration details, see [configure-the-kafka-client-and-gateway.md](configure-the-kafka-client-and-gateway.md "mention").
...
> For the full configuration block (kafka:...), see [Configure The Kafka Client And Gateway](configure-the-kafka-client-and-gateway.md).
...
<td>string</td>
<td>File path to the Gateway keystore containing the Gateway private key and certificate</td>
</tr>
<tr>
...
<td>string</td>
<td>File path to the Gateway truststore containing the CAs that signed client certificates</td>
</tr>
<tr>
...
<td>string</td>
<td>Client authentication mode. Supported values: <code>required</code>, <code>request</code>, <code>none</code></td>
</tr>
</tbody>
</table>

{% hint style="warning" %}
`clientAuth: required` is mandatory to enforce mTLS. The Gateway rejects any client connection without a valid certificate.
{% endhint %}
...
> For the full configuration block (security.protocol=SSL...), see [Kafka](../create-and-configure-apis/configure-v4-apis/endpoints/kafka.md).
...
> For Subscription workflow details, see [Configure The Kafka Client And Gateway](configure-the-kafka-client-and-gateway.md).


{% hint style="info" %}
Applications subscribing to Kafka APIs with mTLS plans must contain a client certificate. APIM uses this certificate to identify the application during the Kafka connection. The behavior is identical to classic V4 APIs using mTLS. For detailed instructions on creating applications with client certificates and managing subscriptions, see [mtls.md](../secure-and-expose-apis/plans/mtls.md "mention").
{% endhint %}
