...
{% hint style="warning" %}
Kafka APIs can't have Keyless, mTLS, and authentication (OAuth2, JWT, API Key) plans published together. You can't expose a Keyless plan, an mTLS plan, and an authentication plan simultaneously on the same Kafka API.
{% endhint %}
