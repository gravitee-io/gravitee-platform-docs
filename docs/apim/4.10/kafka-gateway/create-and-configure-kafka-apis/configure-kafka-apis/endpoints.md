...
* **keystore.path**: File path to the Gateway keystore.
* **keystore.password**: Password for the Gateway keystore.
* **truststore.type**: Format of the truststore file.
* **truststore.path**: File path to the Gateway truststore.
* **truststore.password**: Password for the Gateway truststore.
* **clientAuth**: Client authentication mode. Set to `required` to enforce mTLS. The Gateway rejects any client connection without a valid certificate.
...
1. Add an mTLS plan to the Kafka API.
2. Publish the plan.
