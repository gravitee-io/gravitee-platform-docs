...
{% hint style="info" %}
You can also use the [Gravitee Kafka Gateway](../../../kafka-gateway/) to proxy a Kafka cluster using the Kafka native protocol (over TCP). The Kafka Gateway supports advanced capabilities including multi-endpoint configuration and tenant management for endpoint selection.
{% endhint %}
