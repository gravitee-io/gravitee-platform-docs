...
The AI Semantic Caching policy enables semantic caching of responses based on the similarity of request content. Unlike traditional caching mechanisms that rely on exact text matching, this policy uses an embedding model to transform incoming requests into vector representations, then compares them against previously cached vectors in a vector store. If a similar context is found, the cached response is reused, reducing computation costs and latency.
...
4. If a similar vector is found (based on the vector store's similarity threshold), returns the cached response
...
| Parameters<br>`parameters` | array | | `[{"key": "retrieval_context_key", "value": "{#context.attributes['api']}_{#context.attributes['plan']}_{#context.attributes['user-id']}", "encode": true}]` | Metadata parameters to attach to cached vectors (using EL) |
