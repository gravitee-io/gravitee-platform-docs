---
description: Install a Classic Cloud hybrid API Management 4.10 deployment from the .zip distribution. Follow the steps after installing the platform.
metaLinks:
  alternates:
    - .zip.md
---
...
2.  Configure Logstash using the following `logstash.conf` file:
    ...
    filter {
        if [type] != "request" and [type] != "v4-metrics" {
