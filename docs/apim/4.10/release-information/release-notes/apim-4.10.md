...
* Gateway administrators can now route Native Kafka API traffic to different backend endpoints based on the gateway's configured tenant identifier, enabling a single API definition to serve multiple isolated environments without duplication.
