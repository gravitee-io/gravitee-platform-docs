...
API publishers can [create Cache resources](docs/apim/4.0/guides/api-configuration/resources.md#how-to-create-a-resource):

* To cache upstream of a response with the [Cache policy](docs/apim/4.0/reference/policy-reference/cache.md)
* To cache access tokens with the [OAuth2 policy](../../reference/policy-reference/oauth2/)

The default distribution includes the [Cache resource](docs/apim/4.0/guides/api-configuration/resources.md#cache) plugin, which stores content in-memory and is locally managed on each Gateway node of the installation.
...
APIM also supports the Gravitee [Redis Cache resource](docs/apim/4.0/guides/api-configuration/resources.md#cache-redis) plugin based on [Redis](https://redis.io/documentation). This plugin is not in the default distribution, but can be [downloaded](https://download.gravitee.io/#graviteeio-apim/plugins/resources/gravitee-resource-cache-redis/) and deployed with these [instructions](docs/apim/4.0/overview/plugins.md#deployment).
