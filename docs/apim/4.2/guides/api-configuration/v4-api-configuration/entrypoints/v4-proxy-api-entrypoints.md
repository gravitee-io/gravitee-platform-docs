---
description: Configuration guide for v4 Proxy API Entrypoints.
---

# v4 Proxy API Entrypoints

## Configuration

To alter v4 Proxy API entrypoints, select your API, and then select **General** from the **Entrypoints** category in the left-hand nav.

<figure><img src="../../../../.gitbook/assets/virtual host_on (1) (1).png" alt="The Entrypoints page of an API under an out-of-sync banner, with virtual hosts enabled and the context path masked."><figcaption><p>v4 proxy API entrypoint configuration</p></figcaption></figure>

From here, you can:

* Alter existing entrypoints by changing the context path
* Add a new entrypoint by clicking **Add context path** and then adding a new context path
* Delete existing entrypoints by clicking the <img src="../../../../.gitbook/assets/Screen Shot 2023-07-18 at 10.51.56 AM.png" alt="A black cross icon, used to close or remove an item." data-size="line"> icon associated with the entrypoint that you want to delete
* Choose to enable or disable virtual hosts. Enabling virtual hosts requires you to define your virtual host and optionally enable override access.

When you are done, make sure to redeploy the API for your changes to take effect.
