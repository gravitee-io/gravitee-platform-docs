...
To explain tenant usage and behavior, we will build off of our example use case for [sharding tags](broken-reference). A single API can be deployed to many different Gateways and endpoints, but by using sharding tags you can specify the target Gateway(s), and by using tenants you can specify the target endpoint(s).
