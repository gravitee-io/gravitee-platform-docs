...
This guide will walk through how to configure your general Gravitee API Management (APIM) Gateway settings using the `gravitee.yaml` file. As detailed in the [Configuring APIM Components](broken-reference), you can override these settings by using system properties or environment variables
...
You can configure the APIM Gateway [plugins](broken-reference) directory with `plugins.path` configuration property:
...
Users can add plugins not included in APIM's default distribution to this directory. This includes different versions of Gravitee plugins or their own [custom plugins](broken-reference).&#x20;

{% hint style="info" %}
To understand how Gravitee handles duplicate plugins, see plugins [discovery and loading.](broken-reference)
{% endhint %}
...
The property `removePlugins` has been removed from the Helm chart as it is no longer necessary. See [plugin discovery and loading](broken-reference) for more information.
...
If you want to use a custom prefix, you need to follow the following [instructions](../../plugins-and-integrations/plugins/configure-repositories/#use-a-custom-prefix).
...
If you want to use a custom prefix, you need to follow the following [instructions](../../plugins-and-integrations/plugins/configure-repositories/#use-a-custom-prefix-1).
...
Redis Sentinel and Redis SSL configuration options are presented [here](../../plugins-and-integrations/plugins/configure-repositories/#redis).
