...
* **Custom Properties:** Defined at the API level and read-only during the Gateway's execution of an API transaction. You can learn more about how to set an API's custom properties [here](broken-reference).
...
See the [v4 API creation wizard](broken-reference) for more details.
{% endtab %}
...
As an API publisher, you can define [custom properties ](broken-reference)for your API. These properties are automatically injected into the expression language context and can be referenced during an API transaction from the `{#api.properties}` root-level object property.
