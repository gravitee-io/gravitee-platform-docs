...
The default APIM Gateway distribution requires [MongoDB](docs/apim/4.2/getting-started/configuration/repositories/mongodb.md) to poll the environment configuration and [Elasticsearch](docs/apim/4.2/getting-started/configuration/repositories/elasticsearch.md) for reporting and analytics. See the vendor documentation for supported versions.
...
{% hint style="info" %}
Both the Management API nodes run by default. You can configure APIM to run only one or the other, as described in the [Management API configuration](docs/apim/4.2/getting-started/configuration/apim-management-api/general-configuration.md) section.
{% endhint %}
