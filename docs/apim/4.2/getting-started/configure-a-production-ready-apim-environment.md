...
APIM includes many other configuration options and every environment is unique. However you configure new settings (via the `gravitee.yml` file, APIM Console, or environment and system variables) it is important to understand that one configuration type can override another. The section on configuring the [APIM Gateway](../apim-components/the-gravitee-api-gateway/) gives a good overview of this topic.
{% endhint %}
