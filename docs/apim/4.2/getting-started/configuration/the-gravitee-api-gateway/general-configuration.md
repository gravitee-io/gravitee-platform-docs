...
You can configure the APIM Gateway [plugins](docs/apim/4.2/overview/plugins.md) directory with `plugins.path` configuration property:
...
Users can add plugins not included in APIM's default distribution to this directory. This includes different versions of Gravitee plugins or their own [custom plugins](docs/apim/4.2/guides/developer-contributions/custom-plugins.md).&#x20;

{% hint style="info" %}
To understand how Gravitee handles duplicate plugins, see plugins [discovery and loading.](docs/apim/4.2/overview/plugins.md#discovery-and-loading)
{% endhint %}
...
The property `removePlugins` has been removed from the Helm chart as it is no longer necessary. See [plugin discovery and loading](docs/apim/4.2/overview/plugins.md#discovery-and-loading) for more information.
...
Redis Sentinel and Redis SSL configuration options are presented [here](docs/apim/4.2/getting-started/configuration/repositories/redis.md#redis).
