...
* **Expression Language Condition:** Use [Gravitee's Expression Language (EL)](../../gravitee-api-management/gravitee-expression-language.md) to provide a custom condition. The condition is evaluated for every request and the flow is only triggered if it evaluates to `true`.
