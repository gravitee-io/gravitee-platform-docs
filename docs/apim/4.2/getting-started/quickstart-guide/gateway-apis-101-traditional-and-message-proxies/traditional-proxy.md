...
To clarify, the SSL options shown here are for the connection between the Gateway and your backend server. Configuring a custom truststore and keystore will have no impact on client connections to the Gateway. mTLS between clients and the Gateway are [configured at the Gateway level](docs/apim/4.2/getting-started/configuration/the-gravitee-api-gateway/general-configuration.md), not the API level.
{% endhint %}
