...
* [#message-sampling-strategies](logging.md#message-sampling-strategies "mention")
...
* **Windowed count:** Samples a maximum count of consecutive messages during a sliding time window that conforms to an ISO-8601 duration. When the count is reached during that time window, no new messages are logged until the window closes, then messages are logged again.
{% endtab %}
{% endtabs %}
...
* **Windowed count:** Must be a `string` in the format `count/duration`, where count is an integer and duration conforms to ISO-8601 format
...
This section explains the available sampling strategies for message-level logging in v4 message APIs. Sampling is used to avoid excessive resource consumption and is only relevant to v4 message APIs.

### Sampling strategies

Gravitee supports the following sampling strategies:
...
The following table shows the default and limit values for each sampling strategy:
...
### Configure sampling with `gravitee.yml`

{% hint style="info" %}
If a setting is configured in `gravitee.yml`, the corresponding field is disabled in the Management Console.
{% endhint %}

Sampling methods for v4 message APIs can be configured in the `gravitee.yml` file. The `messageSampling` configuration option determines, for each sampling method, whether it can be used, its default value, and its max value:
...
### Configure sampling with Helm

To configure sampling methods using Helm, add the following configuration to your `values.yaml` file:
...
### Configure message body storage

The `reporting.logging.max_size` parameter controls the size of the message body stored in the logs, as well as the size of logged webhook request body (if enabled).

To configure this parameter in `gravitee.yml`:
...
To configure this parameter using Helm:
