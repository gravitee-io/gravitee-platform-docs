...
<figure><img src="../.gitbook/assets/kafka-gateway-configure-the-kafka-gatewa-151-1.png" alt="A diagram of a Kafka client connecting over SSL to a gateway server, where an API resolver uses SNI to route the connection to the matching API and on to the brokers." width="563"><figcaption><p>The proxy obtains the list of brokers from the upstream cluster.</p></figcaption></figure>
...
<figure><img src="../.gitbook/assets/kafka-gateway-configure-the-kafka-gatewa-153-1.png" alt="A diagram of a Kafka client connecting over SSL to a gateway server, showing the per-API broker names the client is given after the API resolver matches it." width="563"><figcaption><p>The gateway returns the list of brokers back to the client, rewritten to use the gateway hostname.</p></figcaption></figure>
