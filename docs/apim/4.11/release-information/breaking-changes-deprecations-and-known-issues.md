...
**PAPI: Unpublished API plans returned with /apis/:apiId/plans** 
When calling the endpoint /apis/:apiId/plans of an API that is not published on the portal and the user has access to it, all of the published plans are returned.

As a workaround, [steps for workaround]

Track the issue here: [PAPI: Unpublished API plans returned with /apis/:apiId/plans](https://github.com/gravitee-io/issues/issues/11243)
