...
* The Developer Portal API catalog now displays all APIs in a single unified view with grid or list display modes, replacing category-based navigation.
* The subscription workflow now includes numbered step headers to guide users through plan selection, application selection, and review.
* Pagination controls across the catalog and application selection screens support configurable page sizes with accessibility improvements.
* Application selection validates existing subscriptions and displays messaging when an application is already subscribed to the selected plan.
<!-- /PIPELINE:APIM-13003 -->
