...
# Secure an MCP-Proxy With Entra ID OAuth

## Overview

This guide explains how to secure an MCP-Proxy using the OAuth plan with Microsoft Entra ID as the identity provider. The OAuth plan validates access tokens issued by Entra ID, ensuring that only authenticated clients can access the proxy.

By completing this guide, you will configure an MCP-Proxy to accept OAuth tokens from Entra ID and validate them against your Entra ID application registration.
...
You will create an MCP Proxy on the Gravitee Gateway to proxy an existing MCP Tool Server. This guide uses the public [Microsoft Learn MCP service](https://learn.microsoft.com/en-us/training/support/mcp) (`https://learn.microsoft.com/api/mcp`) as the backend. You will enable secured access to this service via a Gravitee OAuth2 Plan that uses Entra ID as the OAuth2 identity provider. Finally, you will use VS Code to access the secured MCP Proxy via the Gravitee Gateway using the MCP Authorization [Pre-Registration](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization#preregistration) method.
...
Microsoft Entra ID does not provide a Token Introspection Endpoint (RFC 7662) as part of its OAuth2/OIDC implementation. To secure an API in Gravitee with Entra ID, you must use Gravitee's JWT Plan, which validates tokens in the request against a configured JWKS_URL.

To correctly support the MCP Authorization Specification, certain elements of the OAuth2 specification are required, such as the WWW-Authenticate header (RFC 9728 Section 5.1), Protected Resource Metadata Discovery (RFC 9728 Section 3.2), and Authorization Server Discovery (RFC 9728). These are available in Gravitee's OAuth2 Plan/Policy. A new Gravitee OAuth2 Resource plugin was created specifically for Entra ID (`gravitee-resource-oauth2-provider-entra-id`) to support these requirements.
...
* Access to Microsoft Entra ID to create a new App Registration for the MCP Tool Server Resource
* Gravitee APIM 4.11.2 or later
* `gravitee-policy-oauth2-5.2.0` (or later) plugin (bundled in APIM 4.11.2)
* `gravitee-resource-oauth2-provider-entra-id-1.0.0` (or later) plugin
* Enterprise License (required to use AI Agent Management features, including MCP Proxy)

## Steps

1. [Create a new MCP-Proxy](#step-1-create-a-new-mcp-proxy) with a KEYLESS plan to confirm access
2. [Test public access](#step-2-test-public-access-using-microsoft-vscode) using Microsoft VS Code
3. [Create Entra ID App Registration](#step-3-create-entra-id-app-registration)
4. [Secure MCP-Proxy with OAuth2](#step-4-secure-mcp-proxy-with-oauth)
 1. [Add Entra ID OAuth Resource](#add-the-entra-id-oauth-resource)
 2. [Remove the KEYLESS plan and add a new secure OAuth plan](#add-the-oauth-plan)
5. [Test secured access](#step-5-test-secured-access-using-microsoft-vscode) using Microsoft VS Code

### Step 1: Create a new MCP-Proxy

1. Add a new MCP-Proxy API with the following configuration:
 1. **API Name**: MS Learn MCP
 2. **Version**: 1.0.0
 3. **Entrypoint**: `/ms-learn-mcp`
 4. **Endpoint**: `https://learn.microsoft.com/api/mcp`
 5. **Plan**: KEYLESS Plan
2. Start and deploy your API to your Gateway.

### Step 2: Test public access using Microsoft VS Code

1. Open VS Code and create a new `mcp.json` file in your workspace.
2. Add a new MCP Tool Server (e.g., `demo-ms-learn-mcp`) that points to the URL of your newly created MCP-Proxy API, as shown in the screenshot below:
...
4. Verify that you see a successful response showing "3 tools."
 ...
 {% hint style="success" %}
 This confirms you can access the public MS Learn MCP Tool Server via the Gravitee Gateway.
 {% endhint %}

5. Stop the MCP Tool Server by clicking **Stop**.
...
1. Log into the Azure Portal and navigate to **App Registrations**.
...
4. Click **Register**.
 ...
 {% hint style="success" %}
 The new App Registration is complete. Obtain and store the following values for later use when configuring OAuth:

 1. **Application (client) ID** (e.g., `2553a281-2ebf-4d6c-aea1-1303a1af0dc6`) — this will become part of your 'audience' value
 2. **Directory (tenant) ID** (e.g., `fffe4189-a2fb-4ec8-89ce-f5649e94735e`)
 {% endhint %}

5. Click **Add a Redirect URI** and add the relevant URIs. For VS Code, add two items:
 1. **Platform Type**: Web
...
7. To allow users (from VS Code) to sign into this app and request their consent, add OIDC (OpenID Connect) scopes. Click the **API permissions** menu and add the following Microsoft Graph delegated permissions:
 1. `email`
 2. `offline_access`
 3. `openid`
 4. `profile`
...
8. Microsoft requires at least one scope for successful operation. In the **Expose an API** menu, click **Add a scope**.
 1. You will first be prompted to define the **Application ID URI**. Leave this as default (e.g., `api://2553a281-2ebf-4d6c-aea1-1303a1af0dc6`) or specify a different value.
...
9. Define a new scope called `Tools.Read`, as shown in the screenshot:
...
Remove the KEYLESS plan from your MCP-Proxy API and replace it with an OAuth plan. First, add the Entra ID OAuth Resource, then add the OAuth plan that references the resource.

#### Add the Entra ID OAuth Resource

1. In the Gravitee APIM Console, navigate to your MCP-Proxy API (MS Learn MCP).
2. Under the **Configuration** menu, click the **Resources** tab.
3. Add a new **Microsoft Entra ID Resource**.
...
4. Configure the Microsoft Entra ID resource:
 1. **Name**: EntraID OAuth Resource
 2. **Audience**: Specify your Entra ID **Application (client) ID** (e.g., `api://2553a281-2ebf-4d6c-aea1-1303a1af0dc6`)
...
3. Save your changes, publish the new OAuth plan, and deploy your API configuration changes to the Gateway.
 ...
 {% hint style="success" %}
 You have secured this MCP-Proxy with the OAuth plan.
 {% endhint %}
...
1. Open VS Code and click **Start** to begin accessing the secured MS Learn MCP Tool Server.
 ...
 {% hint style="info" %}
 If you see any other dialog boxes (such as the screenshot below), your Entra ID OAuth Resource has not been configured correctly (e.g., incorrect Tenant ID).
...
2. VS Code will ask for your `client ID` to use with the MCP Authorization Pre-Registration method. Enter the `client ID` of your Entra ID App Registration. VS Code will also ask for an optional `client secret`. Don't supply this value.
...
3. VS Code will inform you that it wants to authenticate to Microsoft:
 ...
 {% hint style="info" %}
 If you see any other messages at this stage, the issue is usually related to misconfiguration of your Entra ID OAuth Resource or OAuth plan.
 {% endhint %}

4. A new browser tab will open where you can sign in with your Entra ID user account or select an existing Entra ID user account.
5. Consent to the needed permissions for VS Code to use your App Registration:
...
6. Verify that you're signed in:
...
7. In VS Code, verify that you see 3 tools available.
 ...
 {% hint style="info" %}
 If you have any errors or don't see the available tools, you typically have a configuration issue in your Entra ID App Registration. Check your VS Code OUTPUT console to troubleshoot.
 {% endhint %}
...
8. To verify access to these tools, use the CHAT to ask what tools are available from the `demo-ms-learn-mcp` tool server:
 ...
 {% hint style="success" %}
 This confirms you can access the secured MS Learn MCP Tool Server via the Gravitee Gateway using Entra ID OAuth.
 {% endhint %}

## Appendix: Reference MCP-Proxy and Entra ID App Registration configurations
