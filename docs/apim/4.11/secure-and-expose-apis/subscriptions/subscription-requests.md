...
API consumers subscribe to APIs with published plans during the application creation process, or after the application is created, through the APIM Console or Developer Portal.

{% hint style="info" %}
Whether an application has an associated `client_id` depends on how it was configured. To subscribe to OAuth2 or JWT plans, the application has a `client_id`.
{% endhint %}
...
5. Search for the API you want to subscribe to. To be searchable, the API consumer needs access to the API — the API is either public or the API consumer is a member of it.
