...
The `attributes` object property contains attributes that are automatically created by the APIM Gateway during an API transaction or added during the execution phase through the [Assign Attributes policy](policy-studio/policies-for-you-apis/a-c/assign-attributes.md). However, attributes fall into one of two categories based on API type:
