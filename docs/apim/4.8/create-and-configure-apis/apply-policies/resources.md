...
{% hint style="warning" %}
The **Cache Redis** plugin is not included in the default APIM distribution, but you can [download](https://download.gravitee.io/#graviteeio-apim/plugins/resources/gravitee-resource-cache-redis/) and [deploy](docs/apim/4.8/plugins/deployment.md#deployment) it. For information on configuring cache in APIM, see [Configure cache](docs/apim/4.8/prepare-a-production-environment/cache.md). For information on configuring the rate limit repository plugin for Redis, see [Redis](docs/apim/4.8/prepare-a-production-environment/repositories/redis.md#redis).
{% endhint %}
...
As of Gravitee 4.0, the ability to use Confluent Schema Registry as a resource is an [Enterprise Edition](docs/apim/4.8/readme/enterprise-edition.md) capability. To learn more about Gravitee Enterprise, and what's included in various enterprise packages, please:
