...
* [**Data logging masking**](../../reference/policy-reference/template-policy-rework-structure-8.md)**:** If you enable logging on APIs, you can use the data logging masking policy to configure rules to conceal sensitive data.
* [**Assign metrics**](<../../reference/policy-reference/template-policy-rework-structure-4 (1).md>)**:** Use the assign metrics policy to push extra metrics in addition to the natively provided request metrics. These metrics can be used for monetization invoices, analytics dashboards to create custom widgets, and, optionally, to apply aggregations based on their value.
