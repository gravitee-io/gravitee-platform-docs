...
* **OAuth2 - Generic Authorization Server:** Defined to introspect an **access\_token** generated by a generic OAuth2 authorization server. This resource provides a complete configuration about the way to apply token introspection.
* should be able to handle common authorization server from the market by providing
