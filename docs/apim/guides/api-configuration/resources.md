...
* Confluent Schema Registry: The resource is used to get serialization/deserialization data from a Confluent schema registry
* HTTP Authentication Provider: Gravitee.io authentication provider resource used to validate user's credentials against an HTTP server
* Inline Authentication Provider: defined to authenticate a user in memory
* Keycloak Adapter: defined to introspect an access token&#x20;
