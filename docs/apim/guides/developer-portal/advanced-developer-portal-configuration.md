...
Accessing the developer portal directly from the management UI automatically signs you in with the same account. However, the power of the developer portal revolves around exposing your APIs to both internal and external consumers. This necessitates the ability to create new accounts which requires some additional configuration. This section walks administrators through everything they need to know about user management.
...
2. Simple mail transfer protocol (SMTP) configuration

As detailed in [General settings](advanced-developer-portal-configuration.md#general-settings), the **Allow User Registration** option is already enabled by default. However, emailing is disabled since it requires configuring an SMTP email service which is used to confirm user account creation. To configure SMTP, navigate to **Settings** in the management UI and in the secondary sidebar, select **Settings** under the **Portal** header in the submenu.

<figure><img src="https://europe1.discourse-cdn.com/business20/uploads/graviteeforum/optimized/2X/d/d033d41925c00cd1b56d9934ca774e89d4d118a2_2_690x312.png" alt=""><figcaption></figcaption></figure>
...
This configuration file is known as the `gravitee.yml` file. It is one of [three ways to configure settings for your APIM environment](https://docs.gravitee.io/apim/3.x/apim\_installguide\_rest\_apis\_configuration.html#overview): environment variables, system properties, and `gravitee.yml`, in ascending order of precedence. We will be overriding these settings using environment variables, but first, we want to show you where in our docker installation the `gravitee.yml` file for APIM lives.
