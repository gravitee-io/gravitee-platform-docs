...
Next, use the [Gravitee Helm Chart](docs/apim/4.1/getting-started/install-guides/install-on-kubernetes/configure-helm-chart.md) to install the Gateway that will act as an Ingress runtime on your Kubernetes cluster. Below is the minimum set of properties that your Helm values should contain:
...
The Gravitee Kubernetes Operator that will act as our Ingress controller can also be installed using the Gravitee Helm Chart. You can find the operator Helm Chart documentation [here](docs/apim/4.1/getting-started/install-guides/install-on-kubernetes/install-gravitee-kubernetes-operator.md).
...
This example below creates a template that defines a [`cache` policy](docs/apim/4.1/reference/policy-reference/cache.md):
