...
The `attributes` object property contains attributes that are automatically created by the APIM Gateway during an API transaction or added during the execution phase through the [Assign Attributes policy](docs/apim/4.1/reference/policy-reference/assign-attributes.md). However, attributes fall into one of two categories based on API type:
...
See the [v4 API creation wizard](docs/apim/4.1/guides/create-apis/how-to/v4-api-creation-wizard.md) for more details.
{% endtab %}
