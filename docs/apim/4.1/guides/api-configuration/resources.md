...
{% hint style="warning" %}
The **Cache Redis** plugin is not included in the default APIM distribution, but you can [download](https://download.gravitee.io/#graviteeio-apim/plugins/resources/gravitee-resource-cache-redis/) and [deploy](docs/apim/4.1/overview/plugins.md#deployment) it. For information on configuring cache in APIM, see [Configure cache](docs/apim/4.1/getting-started/configuration/configure-cache.md). For information on configuring the rate limit repository plugin for Redis, see [Redis](docs/apim/4.1/getting-started/configuration/configure-repositories/redis.md#redis).
{% endhint %}
