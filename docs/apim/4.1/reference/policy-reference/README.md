...
Each version of Gravitee API Management (APIM) includes a number of policies in the default distribution. [Gravitee Enterprise Edition](../../overview/ee-vs-oss/README.md) policy plugins are available for download [here](https://download.gravitee.io/).
...
Policies can be added to flows that are assigned to an API or to a plan. Gravitee supports configuring policies [through the Policy Studio](../../guides/policy-design/README.md) in the Management Console or interacting directly with the Management API.
...
The [changelog for each version of APIM](../../releases-and-changelog/changelog/README.md) provides a list of policies included in the default distribution. The documentation for each policy includes the compatibility matrix for APIM and that particular policy.
