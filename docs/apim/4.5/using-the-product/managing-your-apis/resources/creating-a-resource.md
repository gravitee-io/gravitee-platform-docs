---
description: Configuration and usage guide for creating a resource.
---

# Creating a resource

To create a resource:

1. Log in to APIM Management Console.
2. Click **APIs** in the left sidebar.
3. Select the API you want to add the resource to.
4. Click **Resources** in the inner left sidebar.
5.  Select the resource you want to configure:

    <figure><img src="../../../.gitbook/assets/create resource.png" alt="The Manage global resources page, empty, with cards offering cache, Redis cache, Confluent schema registry, OAuth2 authorization server, and HTTP authentication provider resources."><figcaption><p>Add a resource</p></figcaption></figure>
6. In the CREATE RESOURCE form, configure the resource per the [Configuration options](creating-a-resource.md#configuration-options) section.
