...
`allow-origin: '*'` should be considered a security risk because it permits all cross-origin requests. **We highly recommend fine-tuning the allow-origin setting. Refer to** the [Gravitee documentation](../../using-the-product/managing-your-apis/api-configuration/v2-api-configuration/general-proxy-settings.md) for other useful information related to CORS.
