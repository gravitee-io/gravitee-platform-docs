...
Deploying a plugin is as easy as copying the plugin archive (zip) into the dedicated directory. By default, you need to deploy the archives in `${GRAVITEE_HOME/plugins}`. Refer to [APIM Gateway Configuration ](../../../using-the-product/using-the-gravitee-api-management-components/apim-gateway/general-configuration.md#configure-the-plugins-directory)for more information on modifying the directory structure.
