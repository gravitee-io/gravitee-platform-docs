...
4. Navigate to the **Membership** section. You can view the API Products that that user is a member of. This section lists all API Products a user is a member of, either directly or through a group, with the role on each. This information is useful to audit a user's effective access without opening each API Product individually.
...
2. From the ** Add a group with roles** pop-up menu, assign the user to a group and one or more roles by completing the following steps:
 ...
 e. From the **Application Role** dropdwon menu, select the Application Role for the user. 
 f. From the **Integration Role** dropdwon menu, select the Integration Role for the user.
 g. Cick **Save**.
