---
description: >-
  Gateway Cluster sync uses Redis to synchronize API Management 4.12 state
  across gateways. Follow the steps to configure it with Helm.
---
...
Gateway Cluster sync leverages Redis to synchronize the state of APIs, keys, and other configurations across your API Gateways in memory, significantly improving system scalability and resilience. Instead of every gateway directly calling the control plane (i.e.: a Bridge Gateway or Management API), a primary gateway node fetches the state and stores it in a Redis-backed Distributed Sync repository, from which all secondary nodes read. This architecture minimizes the load on the main database and ensures high availability, allowing gateways to continue serving traffic - and new instances to start - even if the central control plane experiences downtime. By actively tracking synchronization states and deployment events, the cluster maintains consistent, incremental updates and supports seamless failovers if the primary node goes offline.

#### Example Scenarios

These two scenarios are based on deploying Gateways into Kubernetes with the built-in startup probe.

**Scenario A: rolling restart while the Control Plane is down**
...
**Scenario B: cold start from zero pods while the Control Plane is down**
