...
| `pkcs12` | Loads in memory, but writing a keystore fails on the integrity MAC, which needs `HmacPBESHA256` — an algorithm with no provider once `SunJCE` is gone, surfacing as `calculateMac failed: Algorithm HmacPBESHA256 not available`. |
...
In practice, **configure your keystores and truststores as `pem`**. It needs no password and no conversion tooling. On 4.12.19 and later, `bcfks` is the alternative when you would rather ship one container file; earlier 4.12 patches do not accept the type.
