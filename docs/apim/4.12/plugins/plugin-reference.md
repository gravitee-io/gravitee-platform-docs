...
| Attributes | Default | Mandatory | Support EL | Support secret | Description |
| --- | --- | --- | --- | --- | --- |
| protocol | PLAINTEXT | No | No | No | Define your Kafka-specific authentication flow (PLAINTEXT, SASL_PLAINTEXT, SASL_SSL, and SSL). |
| sasl.saslMechanism | N/A | No | No | No | Define the Sasl mechanism (GSSAPI, OAUTHBEARER, PLAIN, SCRAM_SHA-256, or SCRAM-SHA-512). |
...
[IMPORTANT]
Providing an access token is more suitable when this token does not expire.
For refreshable tokens, it's recommended to use the first way.

####### Token endpoint URL
The recommend way to configure OAUTHBEARER mechanism is to provide a token endpoint URL and define a JAAS configuration like this:
```
...
####### Access token
To provide your custom accessToken, the following JAAS configuration has to be used:
```
...
This mechanism is only available with SASL_SSL protocol. Once selected, you need to provide a valid JAAS configuration.
...
- If the client wants to specify a particular credential profile as part of the client configuration rather than through the environment variable AWS_PROFILE, they can pass in the name of the profile in the JAAS configuration:
...
This is a Solace endpoint which allow subscribing or publishing messages to a Solace broker.
Note that only SMF protocol is supported.
...
In order to use this _Advanced_ version, you only have to declare the following identifier `sse-advanced` while configuring your API entrypoints.
You could also update existing API, thanks to compatibility of the _Advanced_ version configuration with the _OSS_ version
...
[NOTE]
====
*metadataAsComment*: Allow sending messages metadata to client as SSE comments. Each metadata will be sent as extra line following ':key=value' format

*headersAsComment*: Allow sending messages headers to client as SSE comments. Each header will be sent as extra line following ':key=value' format
====
...
In order to use this _Advanced_ version, you only have to declare the following identifier `webhook-advanced` while configuring your API entrypoints.
You could also update existing API, thanks to compatibility of the _Advanced_ version configuration with the _OSS_ version
...
| signatureAlgorithm | RSA_RS256 | Yes | The `alg` header identifying the security mechanism for signing the JWT at the issuer. Supported values: `RSA_RS256`, `HMAC_HS256`, `HMAC_HS384`, `HMAC_HS512`. |
...
* Each role must have the minimum permissions documented in the <<AWS prerequisites>> section
...
This plugin is part of the Federation feature.
It allows to discover, ingest and subscribe to Azure API Management RestApis (only rest are supported by this plugin).
This plugin should be deployed in the Federation Agent.
...
This plugin is part of the Federation feature.
It allows to discover, ingest and subscribe to Confluent platform (only rest are supported by this plugin).
This plugin should be deployed in the Federation Agent.
...
.Configuration
[cols="5*", options=header]
| Property | Required | Description | Type | Default | id | X | The id of the integration | string | CLUSTER_API_ENDPOINT | X | endpoint of cluster API (rest-api) | string | SCHEMA_REGISTRY_ENDPOINT | X | endpoint of schema registry | string | BASIC_AUTH_LOGIN |  | login of both cluster API & schema registry | string | BASIC_AUTH_PASSWORD |  | password of both cluster API & schema registry | string | TOPIC_PREFIX |  | prefix filter of topics | string | TRUST_ALL |  | disable all TLS validation (use with only for dev) | string |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
This plugin is part of the Federation feature. It allows discovering, ingesting and subscribing to Edge Stack APIs. It
also supports discovering and ingesting Emissary (OSS) APIs. This plugin should be deployed in the Integration Agent.
...
Installation of the Gravitee Edge Stack Federation Agent into the Kubernetes cluster or a KUBECONFIG file with
permissions to get and list the `getambassador.io` CRDs and get and update `secrets`.
...
This plugin is part of the Federation feature.
It allows to discover, ingest and subscribe to IBM API Connect apis.
This plugin should be deployed in the Federation Agent.
...
This plugin is part of the Federation feature.
It allows to discover, ingest and subscribe to Mulesoft platform.
This plugin should be deployed in the Federation Agent.
...
We are configuring MongoDB password. So you must Gravitee with MongoDB using a password.
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Cache condition<br>`cacheCondition`| string|  | | (optional) default: {#response.status >= 200 && #response.status < 300}|
| Embedding model resource<br>`modelName`| string| ✅| | Select the AI Model Text Embedding resource configured for this API.|
...
| Prompt expression<br>`promptExpression`| string|  | | (optional) default: {#request.content}|
| Vector store resource<br>`vectorStoreName`| string| ✅| | Select the AI Vector Store resource configured for this API.|
...
[cols="2*", options="header"]
| Key | Parameters | API_KEY_MISSING | - | API_KEY_INVALID_KEY | - |
| --- | --- | --- | --- | --- | --- |
...
| Key | Default status | Parameters | AWS_LAMBDA_INVALID_RESPONSE | 500 | - | AWS_LAMBDA_INVALID_STATUS_CODE | 400 | - |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Masking rules for payload<br>`bodyRules`| array|  | | <br/>See "Masking rules for payload" section.|
| Masking rules for headers<br>`headerRules`| array|  | | <br/>See "Masking rules for headers" section.|
| Scope<br>`scope`| enum (string)|  | `REQUEST_CONTENT`| Execute policy on <strong>request</strong> or <strong>response</strong> phase.<br>Values: `REQUEST_CONTENT` `RESPONSE_CONTENT`|


#### Masking rules for payload (Array)
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Value selector by key<br>`path`| string|  | | Can use json-path for json content or xml-path for xml content|
| Custom value selector<br>`regex`| string|  | | Can use regular expression)|
| Replacement character<br>`replacer`| string|  | `*`| |
| Value sector by type<br>`type`| enum (string)|  | `CUSTOM`| Predefined value selector<br>Values: `CUSTOM` `CREDIT_CARD` `EMAIL` `IP` `URI`|


#### Masking rules for headers (Array)
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Header name<br>`path`| string| ✅| | |
| Replacement character<br>`replacer`| string|  | `*`| |
...
*Mask JSON request content and headers*
```json
{
  "api": {
    "definitionVersion": "V4",
...
```
*Mask XML request content and headers*
```json
{
  "api": {
    "definitionVersion": "V4",
...
[cols="2*", options="header"]
| Key | Parameters | HTTP_SIGNATURE_IMPOSSIBLE_GENERATION | - |
| --- | --- | --- | --- |
...
| Phase | Code | Error template key | Description |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- |
| * | ```400``` | GRAPHQL_RATE_LIMIT_REACH_MAX_COST | When the query reach the max cost | * | ```429``` | GRAPHQL_RATE_LIMIT_TOO_MANY_REQUESTS | When too many requests has been done according to rate limiting configuration |
...
[cols="2*", options="header"]
| Key | Parameters | HTTP_SIGNATURE_INVALID_SIGNATURE | - |
| --- | --- | --- | --- |
...
[cols="2*", options="header"]
| Key | Parameters | JSON_INVALID_PAYLOAD | - | JSON_INVALID_FORMAT | - | JSON_INVALID_RESPONSE_PAYLOAD | - | JSON_INVALID_RESPONSE_FORMAT | - |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| Phase | Code | Error template key | Description |  |  |  |  |  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| onRequest | ```400``` | JSON_INVALID_PAYLOAD | Request payload cannot be transformed properly to XML | onResponse | ```500``` | JSON_INVALID_PAYLOAD | Response payload cannot be transformed properly to XML | onMessageRequest | ```400``` | JSON_INVALID_MESSAGE_PAYLOAD | Incoming message cannot be transformed properly to XML | onMessageResponse | ```500``` | JSON_INVALID_MESSAGE_PAYLOAD | Outgoing message cannot be transformed properly to XML |
...
- ✅🧪 ApiKeys.METADATA
- ✅🧪 ApiKeys.LEADER_AND_ISR -> Note: Removed in 4.x
- ✅🧪 ApiKeys.STOP_REPLICA -> Note: Removed in 4.x
- ✅🧪 ApiKeys.UPDATE_METADATA -> Note: Removed in 4.x
- ✅🧪 ApiKeys.CONTROLLED_SHUTDOWN -> Note: Removed in 4.x
- ✅🧪 ApiKeys.OFFSET_COMMIT
- ✅🧪 ApiKeys.OFFSET_FETCH -> Note: Adaptation required for 4.x
...
- 🤨 ApiKeys.INITIALIZE_SHARE_GROUP_STATE -> TODO(4.x): To add with next version of Kafka when it will be implemented https://github.com/apache/kafka/blob/8e44ddccb5b3335e3450cb46e6be58130f8851a1/core/src/main/scala/kafka/server/KafkaApis.scala#L3168[src]
- ✅🧪 ApiKeys.READ_SHARE_GROUP_STATE
- ✅🧪 ApiKeys.WRITE_SHARE_GROUP_STATE
- 🤨 ApiKeys.DELETE_SHARE_GROUP_STATE -> TODO(4.x): To add when it's implemented in Kafka. https://github.com/apache/kafka/blob/8e44ddccb5b3335e3450cb46e6be58130f8851a1/core/src/main/scala/kafka/server/KafkaApis.scala#L3217[src]
- 🤨 ApiKeys.READ_SHARE_GROUP_STATE_SUMMARY -> TODO(4.x): To add when it's implemented in Kafka https://github.com/apache/kafka/blob/8e44ddccb5b3335e3450cb46e6be58130f8851a1/core/src/main/scala/kafka/server/KafkaApis.scala#L3224[src]
...
- Individual fields — Applies cryptographic operations to specific fields within the payload, identified by their paths (e.g., JSONPath) and original data types. Each field is processed independently.
  Note: Currently, only JSON objects are supported.
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Description  |
|:----------------------|:-----------------------|:----------:|:-------------|
| Compression configuration<br>`compressionConfiguration`| object| ✅| <br/>See "Compression configuration" section.|
| Decryption configuration<br>`decryption`| object|  | <br/>See "Decryption configuration" section.|
| Encryption configuration<br>`encryption`| object|  | <br/>See "Encryption configuration" section.|
| Keystore configuration<br>`keyStore`| object| ✅| All fields support EL and secrets<br/>See "Keystore configuration" section.|
| Metadata configuration<br>`metadataConfiguration`| object| ✅| <br/>See "Metadata configuration" section.|
| Scope configuration<br>`processingScope`| object| ✅| Processing scope for cryptographic operations<br/>See "Scope configuration" section.|


#### Compression configuration (Object)
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Compression algorithm<br>`compressionAlgorithm`| enum (string)| ✅| `NONE`| Algorithm used by compression mechanism to compress & decompress data<br>Values: `NONE` `GZIP` `LZ4_BLOCK` `LZ4_FRAMED` `BZIP2` `SNAPPY_FRAMED`|


#### Decryption configuration (Object)
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Description  |
|:----------------------|:-----------------------|:----------:|:-------------|
| Mode<br>`mode`| object| ✅| Mode of Decryption configuration<br>Values: `DIRECT_RAW_BASE64` `DIRECT_JWE` `JWE_WITH_DEK`|


#### Decryption configuration: Decrypt from base64-encoded raw binary `mode = "DIRECT_RAW_BASE64"` 
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Deciphering algorithm<br>`algorithm`| enum (string)| ✅| `AES_256_GCM`| Deciphering algorithm to be used by the policy<br>Values: `AES_128_GCM` `AES_256_GCM`|
| Key ID<br>`keyId`| string<br>`[1, +Inf]`| ✅| | Key identifier for the decryption key.|


#### Decryption configuration: Decrypt from JWE object with direct decryption `mode = "DIRECT_JWE"` 
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Deciphering algorithm<br>`algorithm`| enum (string)| ✅| `AES_256_GCM`| Deciphering algorithm to be used by the policy<br>Values: `AES_128_GCM` `AES_192_GCM` `AES_256_GCM`|
| Key ID<br>`keyId`| string<br>`[1, +Inf]`| ✅| | Key identifier for the decryption key.|
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Description  |
|:----------------------|:-----------------------|:----------:|:-------------|
| Mode<br>`mode`| object| ✅| Mode of Encryption configuration<br>Values: `DIRECT_RAW_BASE64` `DIRECT_JWE` `JWE_WITH_DEK`|


#### Encryption configuration: Encrypt to base64-encoded raw binary `mode = "DIRECT_RAW_BASE64"` 
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Ciphering algorithm<br>`algorithm`| enum (string)| ✅| `AES_256_GCM`| Ciphering algorithm to be used by the policy<br>Values: `AES_128_GCM` `AES_256_GCM`|
| Key ID<br>`keyId`| string<br>`[1, +Inf]`| ✅| | Key identifier for the encryption key.|


#### Encryption configuration: Encrypt to JWE object with direct encryption `mode = "DIRECT_JWE"` 
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Ciphering algorithm<br>`algorithm`| enum (string)| ✅| `AES_256_GCM`| Ciphering algorithm to be used by the policy<br>Values: `AES_128_GCM` `AES_192_GCM` `AES_256_GCM`|
| Key ID<br>`keyId`| string<br>`[1, +Inf]`| ✅| | Key identifier for the encryption key.|
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Description  |
|:----------------------|:-----------------------|:----------:|:-------------|
| Store Type<br>`storeType`| object| ✅| Store Type of Keystore configuration<br>Values: `NONE` `PKCS12`|


#### Keystore configuration: No keystore `storeType = "NONE"` 
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Key<br>`keyValue`| string| ✅| | Base64 encoded key|


#### Keystore configuration: PKCS12 Keystore with location `storeType = "PKCS12"` 
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Key Password<br>`keyPassword`| string|  | | Password to access the key inside the keystore|
| Location<br>`location`| string| ✅| | Location of the PKCS12 keystore file|
| Password<br>`password`| string| ✅| | Password to access the keystore|


#### Keystore configuration: Base64 encoded PKCS12 Keystore `storeType = "PKCS12"` 
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Content<br>`content`| string| ✅| | Base64 encoded content of the PKCS12 keystore file|
| Key Password<br>`keyPassword`| string|  | | Password to access the key inside the keystore|
| Password<br>`password`| string| ✅| | Password to access the keystore|
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Description  |
|:----------------------|:-----------------------|:----------:|:-------------|
| Placement Type<br>`placementType`| object| ✅| Placement Type of Metadata configuration<br>Values: `NONE` `KAFKA_HEADER`|
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Metadata header prefix<br>`headerPrefix`| string| ✅| `X-Gravitee-Encryption-`| Prefix to use for metadata headers|


#### Scope configuration (Object)
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Description  |
|:----------------------|:-----------------------|:----------:|:-------------|
| Scope Type<br>`scopeType`| object| ✅| Scope Type of Scope configuration<br>Values: `WHOLE_PAYLOAD` `JSON_INDIVIDUAL_FIELDS`|
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Fields<br>`fields`| array<br>`[1, +Inf], unique`| ✅| | List of JSON fields to be processed as JSON path expressions<br/>See "Fields" section.|


#### Fields (Array)
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Field path<br>`fieldPath`| string|  | | |
| Field Type<br>`fieldType`| enum (string)|  | `STRING`| Values: `STRING` `NUMERIC` `BOOLEAN` `OBJECT` `LIST`|
...
*Direct raw encryption with AES 128 GCM and GZIP compression*
```yaml
apiVersion: "gravitee.io/v1alpha1"
kind: "ApiV4Definition"
metadata:
...
```
*Direct JWE encryption with AES 256 GCM and LZ4 compression*
```yaml
apiVersion: "gravitee.io/v1alpha1"
kind: "ApiV4Definition"
metadata:
...
```
*JWE with DEK encryption with AES 256 GCM and Snappy compression*
```yaml
apiVersion: "gravitee.io/v1alpha1"
kind: "ApiV4Definition"
metadata:
...
```
*JWE with DEK decryption with PKCS12 keystore*
```yaml
apiVersion: "gravitee.io/v1alpha1"
kind: "ApiV4Definition"
metadata:
...
```
*Direct raw decryption with individual JSON field processing*
```yaml
apiVersion: "gravitee.io/v1alpha1"
kind: "ApiV4Definition"
metadata:
...
This policy is applied to the `SUBSCRIBE` phase of a Native Kafka API.
It runs on each Kafka message fetched by a consumer before it is forwarded to the client.
...
A subscription can carry arbitrary metadata (set at subscription time in the plan).
The filter expression below keeps only messages that match the `keyword` metadata value
associated with the consumer's subscription:
...
The policy supports double-evaluation: if the `filter` expression resolves to a
string that is itself an EL expression, it is evaluated a second time against the
current message. This allows the filter to be driven by a Kafka message header:
...
It allows you to set a byte-rate threshold for both producers and consumers.
...
Each connection has its own rate limit, with no coordination between them.
Connections are not subject to rate limits until they have been active for at least one second.
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Description  |
|:----------------------|:-----------------------|:----------:|:-------------|
| ACLs<br>`authorizations`| array|  | Define ACLs (Access Control Lists) in order to determine what users are having access to what resources.<br/>See "ACLs" section.|


#### ACLs (Array)
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Description  |
|:----------------------|:-----------------------|:----------:|:-------------|
| Features<br>`authorizedFeatures`| array<br>`[1, +Inf], unique`| ✅| <br/>See "Features" section.|
| Condition<br>`condition`| string|  | The condition for which the following ACL should be be applicable (supports EL).|


#### Features (Array)
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Description  |
|:----------------------|:-----------------------|:----------:|:-------------|
| Type<br>`type`| object| ✅| Type of <br>Values: `TOOLS` `RESOURCES` `PROMPTS`|


#### : Tools `type = "TOOLS"` 
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Tool methods<br>`methods`| array (enum (string))<br>`unique`| ✅| `[TOOLS_LIST]`| |
| Name Pattern Type<br>`patternType`| enum (string)|  | `ANY`| Values: `ANY` `LITERAL` `EXPRESSION`|


#### : Resources `type = "RESOURCES"` 
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Resource methods<br>`methods`| array (enum (string))<br>`unique`| ✅| `[RESOURCES_LIST]`| |
| Name Pattern Type<br>`patternType`| enum (string)|  | `ANY`| Values: `ANY` `LITERAL` `EXPRESSION`|
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Prompt methods<br>`methods`| array (enum (string))<br>`unique`| ✅| `[PROMPTS_LIST]`| |
| Name Pattern Type<br>`patternType`| enum (string)|  | `ANY`| Values: `ANY` `LITERAL` `EXPRESSION`|
...
*Configure the MCP ACL policy to allow only the `get_weather` tool to be listed and called*
```json
{
  "api": {
    "definitionVersion": "V4",
...
* update CONTENT_LENGTH when policy modify response json ([9c62a6e](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/9c62a6ea14a28c43eeb93df1dfac1f3a3fc2b44d))
...
* impl ACL for PROMPTS_GET MCP Method ([335622c](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/335622c38fcb1a3144554fc53b2d150e20bcd923))
* impl ACL for PROMPTS_LIST MCP Method ([77678e1](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/77678e1a1c655e9330a174754b9992fb4608de70))
* impl ACL for RESOURCES_LIST MCP Method ([3add58b](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/3add58b03b8b1659ab636a5b44db57eeb456219c))
* impl ACL for RESOURCES_READ MCP Method ([377f37e](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/377f37ecec1ee4e77672b50f5315000a780c02d4))
* impl ACL for RESOURCES_SUBSCRIBE MCP Method ([8ce04fa](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/8ce04fafb7fe896b4c161c265b10a4aa5cf0fbca))
* impl ACL for RESOURCES_TEMPLATES_LIST MCP Method ([32ca595](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/32ca5951f1f14bf20431fd71069491f582f2515d))
...
* update CONTENT_LENGTH when policy modify response json ([69864d0](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/69864d089f38a1662bcea40f57f8dd6799d4eb3a))
...
* impl ACL for PROMPTS_GET MCP Method ([e64ebe8](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/e64ebe8156f88ecefb74ea88990a85933aec6cff))
* impl ACL for PROMPTS_LIST MCP Method ([f2f23f1](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/f2f23f1a2b4f9640c4eb3df1ff82bcac71859549))
* impl ACL for RESOURCES_LIST MCP Method ([e17cce8](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/e17cce84ee3fd2eac7bc2f09fa41aacf379d4374))
* impl ACL for RESOURCES_READ MCP Method ([94f400d](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/94f400d71c7c0f1fb05ea8b5957f440599a9a088))
* impl ACL for RESOURCES_SUBSCRIBE MCP Method ([9e06ac3](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/9e06ac3db59ccf2995102b900242e3274f3b747d))
* impl ACL for RESOURCES_TEMPLATES_LIST MCP Method ([f55ce44](https://github.com/gravitee-io/gravitee-policy-mcp-acl/commit/f55ce445e20fc8e7bb7583042f51869155ff7a4c))
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Description  |
|:----------------------|:-----------------------|:----------:|:-------------|
| Blacklist<br>`blacklist`| array ()|  | List of client disallowed to connect.|
| Whitelist<br>`whitelist`| array ()|  | List of clients allowed to connect.|




## Examples
*CRD for Kafka Native API With Defaults*
```yaml
apiVersion: "gravitee.io/v1alpha1"
kind: "ApiV4Definition"
metadata:
...
```
*CIDR based whitelist with single IPs blacklisted*
```yaml
apiVersion: "gravitee.io/v1alpha1"
kind: "ApiV4Definition"
metadata:
...
```
*Expression language based whitelist and blacklist*
```yaml
apiVersion: "gravitee.io/v1alpha1"
kind: "ApiV4Definition"
metadata:
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| REQUEST | ```400 - BAD REQUEST``` | OAS_VALIDATION_ERROR_KEY | Request does not match the OpenAPI Specification | RESPONSE | ```500 - INTERNAL SERVER ERROR``` | NO_OAS_RESOURCE_KEY | No resource configured | REQUEST / RESPONSE | ```500 - INTERNAL SERVER ERROR``` | NO_OAS_PROVIDED_KEY | No OpenAPI Specification provided | RESPONSE | ```500 - INTERNAL SERVER ERROR``` | OAS_VALIDATION_ERROR_KEY | Response does not match the OpenAPI Specification |
...
[cols="2*", options="header"]
| Key | Parameters | OAUTH2_MISSING_SERVER | - | OAUTH2_MISSING_HEADER | - | OAUTH2_MISSING_ACCESS_TOKEN | - | OAUTH2_INVALID_ACCESS_TOKEN | - | OAUTH2_INVALID_SERVER_RESPONSE | - | OAUTH2_INSUFFICIENT_SCOPE | - | OAUTH2_SERVER_UNAVAILABLE | - |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| **NETWORK_IDENTIFIER** | IP addresses, MAC addresses, and URLs. |
| **DEVICE_IDENTIFIER** | Unique hardware IDs like IMEI or Serial Numbers. |
| **FINANCIAL_ACCOUNT** | Credit cards, IBANs, and bank account numbers. |
| **GOVERNMENT_ID** | SSNs, Passports, Driver's Licenses, and National IDs. |
| **VEHICLE_ID** | License plates and Vehicle Identification Numbers (VIN). |
...
| Key| Parameters |
| --- | ---  |
| STREAMING_NOT_ALLOWED| Interrupted with "Streaming is not supported" (400). Occurs when a request contains `"stream": true` while `skipResponsePayloadFiltering` is disabled, preventing un-scanned data leaks. |
| PII_ANALYSIS_FAILED| Interrupted with "PII analysis failed" (500). Occurs if the AI Resource is missing, the model fails to initialize, or the inference service times out. |
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| PII Categories<br>`categories`| array (enum (string))<br>`unique`| ✅| | Select the standard PII categories you want to redact/detect.|
| AI Resource<br>`resourceName`| string| ✅| | Select the AI Model Token Classification resource configured for this API.|
| Skip Response Payload Filtering<br>`skipResponsePayloadFiltering`| boolean|  | | If enabled, PII filtering will be skipped for the response payload. Required for streaming (e.g., LLM Server-Sent Events).|
| Confidence Threshold<br>`threshold`| number<br>`[0, 1]`|  | `0.5`| Minimum AI confidence score (0.0 to 1.0) to trigger redaction.|
...
[cols="2*", options="header"]
| Key | Parameters | RATE_LIMIT_TOO_MANY_REQUESTS | limit - period_time - period_unit | QUOTA_TOO_MANY_REQUESTS | limit - period_time - period_unit | SPIKE_ARREST_TOO_MANY_REQUESTS | limit - period_time - period_unit - slice_limit - slice_period_time - slice_limit_period_unit |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
[cols="2*", options="header"]
| Key | Parameters | HEADER_THREAT_DETECTED | - | PATH_THREAT_DETECTED | - | BODY_THREAT_DETECTED | - |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
[cols="2*", options="header"]
| Key | Parameters | REQUEST_CONTENT_LIMIT_TOO_LARGE | length - limit | REQUEST_CONTENT_LIMIT_LENGTH_REQUIRED | limit |
| --- | --- | --- | --- | --- | --- |
...
| Policy scope | ON_REQUEST | status | X |  |
...
[cols="2*", options="header"]
| Key | Parameters | REQUEST_VALIDATION_INVALID | violations |
| --- | --- | --- | --- |
...
[cols="2*", options="header"]
| Key | Parameters | RESOURCE_FILTERING_FORBIDDEN | path - method | RESOURCE_FILTERING_METHOD_NOT_ALLOWED | path - method |
| --- | --- | --- | --- | --- | --- |
...
[cols="2*", options="header"]
| Key | Parameters | RBAC_NO_USER_ROLE (403) | - | RBAC_INVALID_USER_ROLES (400) | - | RBAC_FORBIDDEN (403) | - |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
[cols="2*", options="header"]
| Key | Parameters | SSL_ENFORCEMENT_SSL_REQUIRED | - | SSL_ENFORCEMENT_AUTHENTICATION_REQUIRED | - | SSL_ENFORCEMENT_CLIENT_FORBIDDEN | name (X.500 name from client certificate) |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| Phase | Code | Error template key | Description |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQUEST | ```400 - BAD REQUEST``` | POLICY_TEMPLATE_ERROR_KEY | An error occurs during request | RESPONSE | ```500 - INTERNAL SERVER ERROR``` | POLICY_TEMPLATE_ERROR_KEY | An error occurs during response |
...
## Usage
This rate limit operates with a **one-request latency**. Since the final token count is only known and updated **after** the
request has been processed and the full response has been delivered.

To manage potential technical failures within the distributed counter system. For example, a database failure,
the strategy is employed:
...
* `ASYNC_MODE` when low latency and high throughput are prioritized. Rate-limiting is applied asynchronously, meaning 
the distributed counter value is not strictly accurate.

You may use the `key` if you need an identifier to count the requests. It can be dynamic. You can use Gravitee Expression Language.
Also, you can enable `useKeyOnly` to ensure that the plan and subscription are ignored.

For example, you could use an Expression Language like `{#request.remoteAddress}` and enable `useKeyOnly` to identify 
per-IP traffic and apply the policy's limit.
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Add response headers<br>`addHeaders`| boolean|  | | Add X-Token-Rate-Limit-Limit, X-Token-Rate-Limit-Remaining and X-Token-Rate-Limit-Reset headers in HTTP response|
| Apply rate-limiting<br>`rate`| object| ✅| | <br/>See "Apply rate-limiting" section.|
| Strategy<br>`strategy`| enum (string)| ✅| `ASYNC_MODE`| Defines the strategy for rate-limiting, including execution mode and behavior on internal errors.<br>Values: `BLOCK_ON_INTERNAL_ERROR` `FALLBACK_PASS_THROUGH` `ASYNC_MODE`|


#### Apply rate-limiting (Object)
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Max tokens (dynamic)<br>`dynamicLimit`| string|  | | Dynamic limit on the number of tokens that can be used (this limit is used if static limit = 0). The dynamic value is based on EL expressions.|
| Key<br>`key`| string|  | | Key to identify a consumer against whom the rate-limiting will be applied. Leave it empty to use the default behavior (plan/subscription pair). Supports EL.|
| Max tokens (static)<br>`limit`| integer<br>`[0, +Inf]`|  | | Static limit on the number of tokens that can be used (this limit is used if the value > 0).|
| Time duration<br>`periodTime`| integer| ✅| `1`| How long to reset the limit|
| Time unit<br>`periodTimeUnit`| enum (string)| ✅| `SECONDS`| Values: `SECONDS` `MINUTES`|
| Use key only<br>`useKeyOnly`| boolean|  | | Only uses the custom key to identify the consumer, regardless of the subscription and plan.|
...
> ℹ️
> The policy allows to transform the key and the value of the message. **But** for the AVRO formatted key to be taken into account, 
> the `gravitee-endpoint-kafka` must be at least in version **5.1.0**
...
> ⚠️
> Disabled fields are shown for backward compatibility.
...
> [WARNING]
> The use of Schema Registry is only available to transform messages on the `onMessageResponse` phase.
...
| Phase | Code | Error template key | Description |
| :--- | :--- | :--- | :--- |
| * | `500` | INVALID_AVRO_TRANSFORMATION | When the transformation fails to be applied to the payload. |
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Description  |
|:----------------------|:-----------------------|:----------:|:-------------|
| Conversion<br>`conversion`| object| ✅| Conversion of <br>Values: `json-to-avro` `avro-to-json`|
| Schema Location<br>`schemaLocation`| object| ✅| Schema Location of <br>Values: `inline` `schema-registry`|


#### : JSON to Avro transformation using an inline schema `conversion = "json-to-avro"` `schemaLocation = "inline"` 
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Key mapping<br>`enableKeyTransformation`| boolean|  | | |
| Value mapping<br>`enableValueTransformation`| boolean|  | `true`| |
| Key mapping<br>`keyMapping`| object|  | | <br/>See "Key mapping" section.|
| Schema Definition<br>`schemaDefinition`| string|  | | |
| Serialization format<br>`serializationFormat`| string| ✅| `simple` (constant)| |
| Value mapping<br>`valueMapping`| object|  | | <br/>See "Value mapping" section.|
...
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Schema Definition<br>`schemaDefinition`| string| ✅| | |
| Source<br>`source`| string|  | | Empty means whole payload. (Support Expression Language)|
...
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Schema Definition<br>`schemaDefinition`| string| ✅| | |
| Serialization format<br>`serializationFormat`| enum (string)| ✅| `simple`| Values: `confluent` `simple`|


#### : Avro to JSON transformation using a schema registry `conversion = "avro-to-json"` `schemaLocation = "schema-registry"` 
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| The name of the Schema Registry resource<br>`resourceName`| string| ✅| | |
| Serialization format<br>`serializationFormat`| enum (string)| ✅| `confluent`| Values: `confluent` `simple`|
...
| Key mapping<br>`keyMapping`| object|  | | <br/>See "Key mapping" section.|
| The name of the Schema Registry resource<br>`resourceName`| string|  | | |
| Header or attribute name<br>`schemaIdEntry`| string|  | | |
| Schema ID Location<br>`schemaIdLocation`| string|  | `inline`| Values: `inline` `header` `attribute`|
| Schema ID<br>`schemaIdValue`| integer|  | | |
| Serialization format<br>`serializationFormat`| enum (string)|  | `confluent`| Values: `confluent` `simple`|
| Value mapping<br>`valueMapping`| object|  | | <br/>See "Value mapping" section.|
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Default  | Description  |
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Schema ID location<br>`schemaSelector`| object| ✅| | <br/>See "Schema ID location" section.|
| Source<br>`source`| string|  | | Empty means whole payload. (Support Expression Language)|
...
| Name <br>`json name`  | Type <br>`constraint`  | Mandatory  | Description  |
|:----------------------|:-----------------------|:----------:|:-------------|
| Schema Id Location<br>`schemaIdLocation`| object| ✅| Schema Id Location of Schema ID location<br>Values: `inline` `header` `attribute`|
...
| Phase | Code | Error template key | Description |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- |
| * | ```500``` | INVALID_PROTOBUF_TRANSFORMATION | When the transform fail to be applied to the payload. | * | ```500``` | UNSUPPORTED_CONFIGURATION_KEY | When the policy configuration is not supported. For example, when the policy needs a schema registry but also use the `simple` serialization format. |
...
[cols="2*", options="header"]
| Key | Parameters | XML_THREAT_DETECTED | - | XML_THREAT_MAX_DEPTH | - | XML_THREAT_MAX_LENGTH | - | XML_THREAT_MAX_ATTRIBUTES | - | XML_THREAT_MAX_ATTRIBUTE_VALUE_LENGTH | - | XML_MAX_CHILD_ELEMENTS | - | XML_THREAT_MAX_TEXT_VALUE_LENGTH | - | XML_THREAT_MAX_ENTITIES | - | XML_THREAT_MAX_ENTITY_DEPTH | - | XML_THREAT_EXTERNAL_ENTITY_FORBIDDEN | - |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
[TRUNCATED: Full document is 804,997 chars. Showing target section "## Reporter" + structural context for surgical patching.]
</search>
<replace>
</details>
...
**Plugin ID**: `gravitee-reporter-otel`

#### OpenTelemetry Reporter

##### Description

The OpenTelemetry reporter exports trace and log data to OpenTelemetry-compatible backends. This plugin is required for exporting runtime log records with trace correlation to Loki and other OTLP-compatible observability platforms.

##### Installation

The `gravitee-reporter-otel` plugin must be installed as a `.zip` file in the gateway's `plugins/` directory. The gateway silently ignores `.jar` files — the plugin must be a `.zip`.

##### Compatibility with APIM

| Plugin version | APIM version |
| --- | --- |
| 1.x | 4.11.x to latest |
...
| gravitee.apim.load_average_1m | CPU load average 1m |
| gravitee.apim.load_average_5m | CPU load average 5m (available depending on the system) |
| gravitee.apim.load_average_15m | CPU load average 15m (available depending on the system) |
...
| Metric | Description |
| --- | --- |
| gravitee.apim.mem_total_in_bytes | Total memory in bytes |
| gravitee.apim.mem_free_in_bytes | Free memory in bytes |
| gravitee.apim.mem_used_in_bytes | Used memory in bytes |
| gravitee.apim.mem_free_percent | Free memory percent |
| gravitee.apim.mem_used_percent | Used memory percent |
...
| Metric | Description |
| --- | --- |
| gravitee.apim.process_timestamp | Process timestamp |
| gravitee.apim.process_percent | Process CPU load |
| gravitee.apim.open_file_descriptors | Number of opened file descriptors associated with the current process |
| gravitee.apim.max_file_descriptors | Maximum number of file descriptors allowed on the system |
...
| Metric | Description |
| --- | --- |
| gravitee.apim.jvm_timestamp | JVM timestamp |
| gravitee.apim.uptime_in_millis | JVM uptime |
| gravitee.apim.heap_used_in_bytes | Heap used in bytes |
| gravitee.apim.heap_used_percent | Heap used in percent |
| gravitee.apim.heap_committed_in_bytes | Heap committed in bytes |
| gravitee.apim.heap_max_in_bytes | Max heap in bytes |
| gravitee.apim.non_heap_used_in_bytes | Non-heap used in bytes |
| gravitee.apim.non_heap_committed_in_bytes | Non-heap committed in bytes |
| gravitee.apim.count | Threads count |
| gravitee.apim.peak_count | Threads peak count |
...
| Metric | Description |
| --- | --- |
| gravitee.apim.api_request_count | Number of requests |
| gravitee.apim.api_response_time_ms | Response time spent to call the backend upstream |
| gravitee.apim.proxy_latency_ms | Latency of the gateway to apply policies |
| gravitee.apim.proxy_response_time_ms | Global response time to process and respond to the consumer |
| gravitee.apim.response_content_length | Size of the response payload |
| gravitee.apim.request_content_length | Size of the request payload |
...
| --- | --- |
| gravitee.apim.api_request_count | Number of requests |
| gravitee.apim.endpoint_response_time_ms | Response time spent to call the backend upstream |
| gravitee.apim.gateway_response_time_ms | Global response time to process and respond to the consumer |
| gravitee.apim.gateway_latency_ms | Latency of the gateway to apply policies |
| gravitee.apim.response_content_length | Size of the response payload |
| gravitee.apim.request_content_length | Size of the request payload |
...
Since the content is parsed, each field can be used in DataDog Log Explorer to filter. If for example, you want to filter on the clientRequest URI "/my_api_uri" with a 200 response status, you can use this search filter: `@clientRequest.uri:"/my_api_uri" @clientResponse.status:200`
