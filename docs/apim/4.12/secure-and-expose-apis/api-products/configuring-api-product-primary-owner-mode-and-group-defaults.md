...
2. Locate the **API Product Primary Owner mode** section.
...
Each group can declare a default API Product role that is applied when the group is attached to an API Product or when a new member is added to the group.

1. Navigate to **Settings** → **Groups**.
2. Select the group you want to configure.
3. In the group details panel, locate the **Default API Product Role** field.
4. Select a role (e.g., **USER**, **OWNER**, or a custom role).
...
When enabled, a group is automatically attached to every API Product created afterwards. Members of the group automatically gain the group's default API Product role on each new product.

1. Navigate to **Settings** → **Groups**.
2. Select the group you want to configure.
3. In the group details panel, enable the **Assign group to all new API Products** toggle.
...
3. In the group details panel, click **Assign group to all existing API Products**.
...
5. In the **API Product Role** field, select the role the member should hold.

Combined with **Assign group to all new API Products** or **Assign group to all existing API Products**, this provides a single place to grant cross-product access.
