---
description: A plan is the service and access layer an API Management 4.12 API needs before you expose it. Compare the plan types available.
metaLinks:
  alternates:
    - ./
---
...
* Ignore an empty `Authorization` header or any type other than Bearer
* An empty Bearer token is considered invalid
* When multiple OAuth2 plans are configured, the Gateway calls the introspection endpoint on **all** configured OAuth2 resources before evaluating selection rules. For more information, see [OAuth2 plan introspection behavior.](oauth2.md#introspection-behaviour-with-multiple-oauth2-plans)
{% endtab %}
