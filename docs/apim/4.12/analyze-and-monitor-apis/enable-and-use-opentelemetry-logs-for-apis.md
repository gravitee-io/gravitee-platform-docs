...
1. In Grafana, open **Explore** → **Tempo** and query logs for your API.
...
4. Click **Logs for this span** to navigate to the correlated log lines in Loki.
    ...
    A pre-filtered log list appears with the detailed payload of all spans.
...
| `analytics.otelLogs.enabled` | Emit request and response payloads as OpenTelemetry log records correlated to the active trace. Capture is subject to the tracing sampling strategy configured in `analytics.sampling`. | `false` |
...
- Log records are always exported over HTTP/protobuf. Loki does not implement the gRPC LogsService, so the `logsEndpoint` must be an HTTP URL (e.g., `http://localhost:3100/otlp/v1/logs`). The SDK does not append the signal path automatically; the full URL including `/v1/logs` must be provided.
...
The `gravitee-reporter-api` dependency is upgraded to `2.3.0` to add trace ID and span ID fields to Request, Response, and MessageLog. The `gravitee-node` dependency is upgraded to `9.0.0-alpha.10` to add the Logger API and OpenTelemetry log emission. The Console UI adds the OTel Logs toggle in the Reporter Settings for both Message APIs and Proxy APIs. The toggle is disabled when Analytics is disabled or when Tracing is disabled. When Tracing is toggled off, OTel Logs is disabled and unchecked. When Tracing is toggled on, OTel Logs is enabled. The `createTracer` method now uses node-level identifiers (`node.id()`, `node.application()`, runtime major version) instead of API-level identifiers. API-level attributes are now passed as additional resource attributes in a map with keys `gravitee.api.id` and `gravitee.api.name`.
