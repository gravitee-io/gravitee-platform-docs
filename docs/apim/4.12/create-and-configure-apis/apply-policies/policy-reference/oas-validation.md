...
### Adding OAS Validation During API Import

When importing or updating a v4 HTTP proxy API from an OpenAPI specification, you can automatically add the OAS Validation policy to generated flows:

1. Open the API import wizard (for new APIs) or click **Import** on an existing v4 HTTP proxy API's details page (for updates).
2. Select **OpenAPI Specification** as the format.
3. Choose your file source (**Local** or **Remote**) and upload or fetch the OpenAPI specification.
4. Enable **With OAS Validation Policy** to attach the policy to generated flows.
5. Click **Import API** (create mode) or **Update API** (update mode).

The imported OpenAPI specification becomes the validation source. Updates from OpenAPI are destructive and overwrite the API's existing configuration.

For complete import workflow details, see [Import or Update a V4 API from OpenAPI or Gravitee Definition](../../create-apis/import-or-update-a-v4-api-from-openapi-or-gravitee-definition.md).
