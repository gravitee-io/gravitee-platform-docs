...
4. (Optional) For each policy step, add a description in the **Description** field. When verbose tracing is enabled on the API and gateway, policy descriptions are included as `gravitee.policy.description` span attributes in distributed tracing data.
5. To persist the changes, click **Save**.
6. To deploy the shared policy group to the gateway, click **Deploy.**
