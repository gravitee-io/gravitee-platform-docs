---
description: A sample compose file that sets up a Classic Cloud hybrid API Management 4.12 configuration. Follow the steps to adapt it.
metaLinks:
  alternates:
    - docker-compose.md
---
...
services:
  ...
  gateway:
    image: graviteeio/apim-gateway:<add_the_gateway_tag> #The gateway version to install. It has to align with the control plane of your Gravitee Cloud
...
* Replace `<add_the_gateway_tag>` in the sample `docker-compose.yml` with the gateway version to install. It has to align with the control plane of your Gravitee Cloud.
