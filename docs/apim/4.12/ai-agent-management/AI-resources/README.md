---
description: The resources that support AI-powered policies in API Management 4.12, covering model inference and vector storage. Compare what each provides.
---
...
The `apiKey` of the OpenAI provider and the value of each HTTP provider header take Gravitee Expression Language, including `{#secrets.get('...')}` secret references.
