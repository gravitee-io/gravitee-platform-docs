...
Gravitee supports three endpoint connector types for Kafka APIs:
...
| **Virtual Cluster** | `native-kafka-virtual-cluster` | API fans out across N backend clusters. | `{ virtualClusterCrossId }` |
...
### Virtual Cluster endpoint configuration

To create a Kafka API with a Virtual Cluster endpoint:

1. In the endpoint configuration step of the wizard, select **Virtual Cluster** as the endpoint connector type.
2. Select a **Virtual Cluster** from the dropdown (filtered to show only `KAFKA_VIRTUAL_CLUSTER` type entities).

Virtual Cluster endpoints are configured with the following property:

| Property | Description | Example |
|:---------|:------------|:--------|
| `virtualClusterCrossId` | Cross-environment identifier of the virtual cluster in the cluster registry. | `vcluster-global-001` |
