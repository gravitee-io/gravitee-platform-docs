...
| --- | --- | --- |
| **API Management** | REST, GraphQL, gRPC, and WebSocket traffic, through API proxies and API Products | [API Management overview](../api-management/get-started/api-management-overview.md) |
...
* **Edge Daemon**. This lightweight process runs on your managed devices. It intercepts the AI agents you choose, forwards their traffic to a target API on the gateway, and reports the providers that a device reaches directly.
...
| Several APIs are published together as one product, with a shared plan and a single subscription. | [Create an API Product](../api-management/build/api-products.md) |
...
| Outcome | Feature |
| --- | --- |
| Every LLM call records the provider, the model that answered, the tokens in and out, and the cost priced from the model's configured rate. | [Monitor your LLM proxy](../agent-management/observe/monitor-your-llm-proxy.md) |
| A team spends against a budget that caps dollars per period, and every member holds a separate API key. | [AI Workspaces](../agent-management/build/ai-workspaces/README.md) |
...
| AI traffic on managed devices that bypasses the gateway entirely becomes visible. | [Monitor detected shadow AI](../edge-management/observe/monitor-shadow-ai-traffic.md) |
...
| Users, groups, and the default roles their members hold are administered for the whole organization. | [Manage users](manage-users.md) |
| Request and response policies apply across the organization's APIs, before and after each API's own flows. | [Manage platform policies](manage-platform-policies.md) |
| Sharding tags route APIs to the gateway groups that should run them. | [Manage entrypoints and sharding tags](manage-entrypoints-and-sharding-tags.md) |
| Every configuration change is recorded at both organization and environment scope. | [Review organization and environment audit logs](review-audit-logs.md) |
...
| LLM APIs | Full | Through the LLM Proxy, with provider-specific routing for OpenAI, OpenAI compatible, Anthropic, Gemini, Bedrock, and Vertex AI |
...
| [**Gateway instances**](monitor-gateway-instances.md) | Which gateway nodes run behind an environment, and how healthy they are. |
| [**Audit logs**](review-audit-logs.md) | Who changed which configuration, at organization and environment scope. |
