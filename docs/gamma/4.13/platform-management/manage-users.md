...
<figure><img src=".gitbook/assets/gamma-platform-users-list.png" alt="Badges mark the primary owner, service accounts, and users holding an active token."><figcaption><p>The Users page lists the users of the organization with their status, identity provider, and last login. Names and email addresses are obscured in this example.</p></figcaption></figure>
...
Either outcome sends the user an email. Accepting someone who hasn't chosen a password yet sends them the activation email, which lets them choose one. For a request sent from the Gamma sign-in page, its link opens the Gamma console, and accepting fails while the organization has no Gamma console URL configured. The user then stays Pending. Every other outcome sends an email that names the decision.
...
<figure><img src=".gitbook/assets/gamma-platform-user-add-to-group.png" alt="A group admin flag, plus separate API, API Product, Application, and Integration roles."><figcaption><p>The Add a group with roles panel. Pick the group, then set the roles the membership grants.</p></figcaption></figure>
...
<figure><img src=".gitbook/assets/gamma-platform-user-group-memberships.png" alt="The Add to Group action, with the APIs and API Products tables both empty."><figcaption><p>The APIs and API Products tables under the Group Memberships card. This user belongs to no group and holds no resource membership in the selected environment.</p></figcaption></figure>
...
<figure><img src=".gitbook/assets/gamma-platform-user-tokens.png" alt="The empty token card, prompting you to generate one for API or CI/CD access."><figcaption><p>The Personal Access Tokens card for a user who holds no token yet. API and application names are obscured in this example.</p></figcaption></figure>
...
<figure><img src=".gitbook/assets/gamma-platform-user-detail.png" alt="The profile card carries the identity provider, last login, and creation date."><figcaption><p>A user's detail page. The assigned role appears under Organization Roles on the profile card and in the Organization Roles picker below it. The name, email address, and environment name are obscured in this example.</p></figcaption></figure>
