...
<figure><img src=".gitbook/assets/gamma-platform-audit-organization.png" alt="Each entry gives the date, user, type, event, and target, with filters and an export action."><figcaption><p>The Audit page of the <strong>Organization</strong> section. The <strong>Reference</strong> column is obscured in this example.</p></figcaption></figure>
...
<figure><img src=".gitbook/assets/gamma-platform-audit-environment.png" alt="The same trail scoped to one environment, with the time filter set to the last 90 days."><figcaption><p>The Audit page of the <strong>Environment</strong> section. The <strong>Reference</strong> column is obscured in this example.</p></figcaption></figure>
...
<figure><img src=".gitbook/assets/gamma-platform-audit-event-type-filter.png" alt="The open list with All events selected, scrolling through alert and API key event types."><figcaption><p>The event type filter, which lists every audit event type the platform defines, sorted by name.</p></figcaption></figure>
...
<figure><img src=".gitbook/assets/gamma-platform-audit-type-filter-organization.png" alt="The open type list with All types selected as the default, and four narrower scopes."><figcaption><p>The type filter on the Organization page, which offers <code>ORGANIZATION</code>, <code>ENVIRONMENT</code>, <code>APPLICATION</code>, and <code>API</code>.</p></figcaption></figure>

<figure><img src=".gitbook/assets/gamma-platform-audit-type-filter-environment.png" alt="The same filter with only two scopes, the organization and environment entries absent."><figcaption><p>The type filter on the Environment page, which offers <code>APPLICATION</code> and <code>API</code> only.</p></figcaption></figure>
...
<figure><img src=".gitbook/assets/gamma-platform-audit-event-panel.png" alt="A page update by the system user, whose JSON Patch is an empty array."><figcaption><p>The <strong>Audit event</strong> panel, which repeats the event's <strong>User</strong>, <strong>Type</strong>, <strong>Reference</strong>, <strong>Event</strong>, and <strong>Target</strong>.</p></figcaption></figure>
...
<figure><img src=".gitbook/assets/gamma-platform-audit-event-panel-patch-add.png" alt="Three add operations set the reference type, the reference, and the identity provider."><figcaption><p>An event that activated an identity provider. Each entry in the <strong>JSON Patch</strong> names a field the change added, with the value it took.</p></figcaption></figure>

<figure><img src=".gitbook/assets/gamma-platform-audit-event-panel-patch-remove.png" alt="Three remove operations name the same paths, each without a value."><figcaption><p>The matching deactivation event, whose <strong>JSON Patch</strong> names the fields the change removed.</p></figcaption></figure>
