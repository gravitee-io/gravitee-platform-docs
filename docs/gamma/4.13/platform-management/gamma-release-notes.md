...
* Dashboard filters are saved in the definition and applied to every widget. **Allow viewers to change value** decides whether a reader gets a locked chip or one they re-value, and a filter saved without a value becomes a slot the reader fills.
