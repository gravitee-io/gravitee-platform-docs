...
Reference values for API products in the Gamma API Management module: the product's own properties, the flag that decides which APIs can be bundled, and the plan types a product supports.
...
An API is only offered to an API product when its `allowedInApiProducts` flag is set. The flag applies to V4 HTTP proxy APIs.

| Property                | Type    | Value in Gamma                                                                                                        | Description                                                                              |
| ----------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `allowedInApiProducts`  | Boolean | `false` for an API proxy created in the Gamma console. Absent (`null`) for API types other than HTTP proxy.            | Makes the API eligible for inclusion in API products.                                    |

An API proxy created through **Create New Proxy** is created with `allowedInApiProducts` set to `false`, so a newly created proxy is not eligible for API products until you enable the flag yourself. When the field is absent from a V4 HTTP proxy API's definition, the Management API also reports it as `false`.

### Enable an API for API products

1. From the API Management sidebar, select **API Proxies**, then select the API.
...
4. Select **Save changes** in the bar that appears in the page header once the form has unsaved edits. A `Changes saved` notification confirms the update.

The switch does nothing until the form is saved — changing it only marks the form dirty, which is what makes the **Discard** and **Save changes** buttons appear next to the **General** heading.

The switch is read-only when you lack update permission on the API definition, or when the API is managed by the Kubernetes operator.
...
APIs of a type other than HTTP proxy — MCP proxies and LLM proxies, for example — do not carry the flag at all and never appear in the picker, regardless of what you search for.

### Turning eligibility off again
...
The constraint is applied when the product's API list is next submitted: creating or updating a product whose `apiIds` include an API with the flag off is rejected with a validation error of the form `These APIs [<ids>] are not allowed in API Products`. The same validation rejects APIs that do not exist, and APIs that do not use the V4 API definition.

## Plan reference model

Plans and subscriptions record the entity they belong to through a pair of reference fields, which is what lets the same plan model serve both an API and an API product.
...
An API product supports a subset of the plan security types available to an API proxy.
...
* [Configure API products](README.md) — Where each configuration area lives in the product sidebar.
* [Create API Products](../api-products.md) — Creating a product and the onboarding checklist.
