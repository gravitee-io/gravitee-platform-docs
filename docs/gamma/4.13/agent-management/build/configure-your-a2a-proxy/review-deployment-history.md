...
hidden: false
noIndex: false
description: Compare two deployed versions of an A2A Proxy definition and roll back to an earlier one. Follow the steps to inspect the history and restore a version.
---

# Review deployment history

The **Deployment History** page lists all deployments of this A2A Proxy, newest first. Select two versions to diff them, or use the actions menu to inspect a single version.
...
{% hint style="warning" %}
The rollback restores the API definition to the selected version, and can't be undone. It doesn't redeploy the A2A Proxy. The gateway keeps serving the live version, and the proxy is marked out of sync until you deploy it again.
{% endhint %}
...
1. Deploy the A2A Proxy twice with different configurations, and label each deployment.
