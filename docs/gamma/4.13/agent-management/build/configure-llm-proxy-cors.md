...
The **CORS** page configures cross-origin access for an LLM Proxy. When CORS is enabled, the gateway adds the `Access-Control-*` headers to the responses of the proxy and answers browser preflight requests itself. CORS is off until you enable it, so a proxy created before the page existed keeps its behavior.
...
2. Send a preflight request to a URL of the proxy, as listed in the **Exposed entrypoints** card of the **Entrypoints** page:
...
* [Configure LLM Proxy entrypoints](configure-llm-proxy-entrypoints.md). Change the context paths the proxy answers on.
