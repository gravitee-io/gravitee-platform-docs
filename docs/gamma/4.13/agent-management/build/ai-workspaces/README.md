...
An AI Workspace gives a team governed access to a chosen set of AI models. You pick the models the workspace exposes, set the budget its members spend against, and add the users who consume it. Every member gets their own application and API key, and is metered individually against the budget you assign them.
