...
hidden: false
noIndex: false
description: Event Stream Management governs the Kafka clusters you already run, through Registered Clusters, Virtual Clusters, and Kafka Services. Learn what it does and how it fits Gamma.
---
...
Within Gamma, Event Stream Management provides a dedicated console for registering Kafka clusters, federating them into Virtual Clusters, publishing governed Kafka Services, and reading live cluster data with Kafka Explorer.
...
Event Stream Management sits between your Kafka infrastructure and the teams, applications, and agents that produce and consume event data. The **Event Gateway** enforces runtime policy on every event interaction, including authentication, authorization, rate limiting, and protocol mediation. The **Gamma console** provides the control plane where you register clusters, compose Virtual Clusters, publish Kafka Services, and inspect live cluster data.
...
* **Kafka Explorer**. Read the live brokers, topics, partitions, consumer groups, and messages behind a saved connection, without writing anything to the cluster. See [Kafka Explorer](../manage/kafka-explorer/README.md).
...
| **Kafka Explorer connection** | A saved pointer at a registered cluster, a Kafka Service, or a broker address, with an optional client-side security overlay. It's read-only, and it's separate from the governance path. | [Create a Kafka Explorer connection](../manage/kafka-explorer/create-a-kafka-explorer-connection.md) |
...
| **Manage** | **Kafka Explorer** | Saved connections, and the read-only views of the brokers, topics, consumer groups, and messages behind them. |
...
| A second service with the same configuration reaches a different listener host prefix, without rebuilding it in the wizard. | [Duplicate a Kafka service](../build/duplicate-a-kafka-service.md) |

### Inspect a cluster without changing it

| Outcome | Feature |
| --- | --- |
| The brokers, topics, partitions, and consumer group lag of a live cluster are visible from the console, with no separate Kafka tooling to install. | [Explore brokers, topics, and consumer groups](../manage/kafka-explorer/explore-brokers-topics-and-consumer-groups.md) |
| A batch of messages is fetched from a topic by position, timestamp, or offset, or new messages are streamed as they arrive. | [Browse and tail topic messages](../manage/kafka-explorer/browse-and-tail-topic-messages.md) |
| Reading a topic leaves no trace, because the explorer commits no offsets and joins no consumer group. | [Limitations and considerations](../manage/kafka-explorer/limitations-and-considerations.md) |
