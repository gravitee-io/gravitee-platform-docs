...
API Management contributes REST, GraphQL, gRPC, and WebSocket APIs to the Catalog. Because the Catalog is shared, those APIs can be exposed as **API Tools** in [Agent Management](../../agent-management/overview/README.md), which makes an accumulated enterprise API estate reachable from the agent layer without redevelopment. [Event Stream Management](../../event-stream-management/get-started/event-stream-management-overview.md) contributes Kafka streams to the same Catalog through the same route.
