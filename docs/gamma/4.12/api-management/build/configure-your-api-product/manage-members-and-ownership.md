...
The **User Permissions** tab controls who can view and manage an API product. It is the only page in the **Security** group of the API Product detail sidebar, and it covers direct members, members inherited from groups, ownership transfer, and membership notifications.

## Membership model

A user gains access to an API product in one of two ways:
...
Detaching a group removes inherited access for that group's members. It does not affect anyone who is a direct member of the product.
...
Every API product has a single primary owner, assigned when the product is created. In the **Direct Members** table, the primary owner's role is shown as a **Primary Owner** badge and the row has no actions menu, so the primary owner cannot be given a different role or removed from the product while they hold that role.
...
2. In the **Add Members** panel, type at least two characters into the search field to look up users by name or email. Users who are already members of the product are excluded from the results.
...
Removing a direct member revokes only their direct access. If they also belong to a group attached to the product, they keep the access that group grants.

## Transfer ownership

Transferring ownership moves the `PRIMARY_OWNER` role to another user and assigns a different product role to the previous primary owner. The action cannot be undone.

1. On the **User Permissions** tab, select **Transfer ownership**.
2. Choose where the new primary owner comes from:
   * **API Member** — select an existing product member from **Select API member**. The current primary owner is not listed.
   * **Other User** — search the organization's users by name or email, typing at least two characters, then select the user from the results.
