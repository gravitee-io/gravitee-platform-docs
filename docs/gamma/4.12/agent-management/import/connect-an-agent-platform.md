...
An integration connects a cloud AI platform once, so the agents it hosts can be imported into the Catalog and kept in step with the platform. Each integration names one project on one platform and the credential every sync signs in with. Its agents come in when you sync it, and a sync mirrors the platform: what the platform lists is imported or refreshed, and what it no longer lists is removed.

The **Integrations** page lists the platforms this environment connected. Two platforms can be connected from the console today: **Azure AI Foundry** and **Gemini Enterprise Agent Platform**. The platform picker also lists **AWS Bedrock**, which can't be connected from this console yet, so selecting it shows no fields.

## Before you start

* Have a credential of the right type registered under **Credentials**, in the **Catalog** section of the sidebar, and allowed on the **Agent Platform** usage. Azure AI Foundry takes an **OAuth2** credential, which mints a token on every call. Gemini Enterprise Agent Platform takes a **GCP service account** credential, which signs a fresh assertion on every read. The integration form lists only the credentials of the right type and usage, and reads **Add one in Credentials** when none exists.
...
4. Under **Platform**, select the platform.
5. Fill in the connection. The fields depend on the platform, as described in the next two sections.
6. Click **Test connection**. The button stays disabled until every field is filled in. When the platform answers, an alert reads **The platform answered** and names the credential that was accepted and how long the call took. For Azure AI Foundry it also names the agents the project hosts, or reads **The project answered but hosts no agent yet.** For Gemini Enterprise Agent Platform it reads **The project answered, so this key can read it.** and the **Regions to import from** field appears.
...
An **Integration connected** notification appears and the list opens again. A test that fails reads **The platform could not be reached** with the platform's reason, and a save that fails reads **The integration was not created**.

<!-- TODO: Screenshot of the Add integration page with Azure AI Foundry selected, the fields filled in, and the platform's answer shown -->

<figure><img src="../.gitbook/assets/PLACEHOLDER-gamma-aim-integrations-add.png" alt=""><figcaption><p>The Add integration page after a passed connection test</p></figcaption></figure>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>The OAuth2 credential the syncs sign in with. Only OAuth2 credentials allowed on agent platforms are listed.</td>
        </tr>
    </tbody>
</table>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td><strong>Credential</strong></td>
            <td>The service account credential the reads sign in with. Only service account credentials allowed on agent platforms are listed.</td>
        </tr>
        <tr>
            <td><strong>Regions to import from</strong></td>
            <td>Shown once the connection test answered, listing the regions the project reports. Pick the ones you use. Left empty, every region the project reports is read, and each region costs one call per sync.</td>
        </tr>
    </tbody>
</table>

No region is asked for before the test. The platform places an agent in one region without being told, and the region each agent lives in is read back when it's imported.
...
The card reads **Syncing agents from &lt;project&gt;…** while the platform is read and the catalog rewritten, which can take a while on a large project. When it's done, the card reads **Last sync just now: &lt;n&gt; imported, &lt;n&gt; updated, &lt;n&gt; removed.** and an **Agents synced** notification appears. A sync that fails reads **Sync failed** with the platform's reason, and the button reads **Retry**. A sync runs only when you trigger it, and the card can't say when the previous one ran.

<!-- TODO: Screenshot of an integration's page with the Details card and the Agents card after a sync -->

<figure><img src="../.gitbook/assets/PLACEHOLDER-gamma-aim-integration-agents-card.png" alt=""><figcaption><p>The Agents card of an integration after a sync</p></figcaption></figure>

The card lists the agents the integration brought in, newest first, 25 per page, with their **State**, **Publication**, **Protocols**, **Model**, and **Imported** date. **State** and **Publication** are the platform's own words for the agent, as of the last sync. Before the first sync the card reads **Nothing imported yet**.

A sync mirrors the platform:

* An agent the platform lists and the Catalog doesn't hold is imported.
* An agent the Catalog already holds is rewritten from what the platform says: its name, description, model, tags, state, endpoints, protocols, skills, version, and security schemes. The URL, provider, and documentation URL you set, the metadata you added, the owner, and whether you stopped the agent survive a sync.
* An agent the platform no longer lists is removed, and only after a listing that completed. A listing that fails part-way removes nothing. On Gemini Enterprise Agent Platform, removal is scoped to the regions the sync managed to read.
...
3. Click **Test connection**, and then **Save changes**. As on creation, the save waits for the platform's answer.
