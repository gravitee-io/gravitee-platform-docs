...
* The header shows the domain as the title with a **Shadow AI** badge, the **Running** and **Discovered** badges, and the **Owner** row, which reads **Nobody**. A **Compliance owner** row appears beside it when the agent's compliance assessment includes the EU AI Act framework's **Accountable human** control, and reads **Not named** while nobody is named.
* The sidebar lists the same sections as a registered agent's page. A section nobody declared is marked **Not declared**, and a section the gateway would have to observe is marked **Not observable**. Opening one shows a sentence on why it's empty, for example that nobody declared an identity, so nothing can authenticate as the agent. **Compliance** opens as it does for a registered agent, under a notice that reads **Nobody declared this agent, so it answers none of these controls**. **Overview** is rebuilt for a detected domain, as the next bullets describe. The bar of actions at the top of a registered agent's page isn't shown.
