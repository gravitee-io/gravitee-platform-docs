...
{% hint style="info" %}
These attributes apply from APIM 4.12.18. APIM 4.12.14 to 4.12.17 report `rpc.system` in place of `rpc.system.name` and `rpc.jsonrpc.request_id` in place of `jsonrpc.request.id`, report the full target URL in `server.address`, and don't report `server.port`. Releases before 4.12.14 don't record an A2A span.
{% endhint %}
