...
{% hint style="info" %}
These attribute names apply from APIM 4.12.18. Earlier 4.12 releases report a different set: `rpc.system` in place of `rpc.system.name`, `rpc.jsonrpc.request_id` in place of `jsonrpc.request.id`, and no `server.port`.
{% endhint %}
