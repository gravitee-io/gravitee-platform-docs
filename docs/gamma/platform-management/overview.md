...
![Gravitee Platform architecture](../.gitbook/assets/gamma-overview-architecture.jpeg)

Gamma unifies five product lines: API Management, Event Stream Management, Agent Management, Authorization Management, and Edge Management under a shared platform. All four share a common Catalog of assets and a common authorization engine that defines fine-grained policies against those cataloged assets. They also share common enforcement points—the AI Gateway, API Gateway, and Event Gateway—that evaluate the same policies at the wire level.


### Platform Components and key functionality
...
Before using Gravitee Gamma, install or activate the platform:

- **New to Gravitee?** See [Get started for new users](get-started/get-started-for-new-users/README.md) to install Gamma using Docker or Kubernetes.
- **Already running Gravitee?** See [Get started for existing users](get-started/get-started-for-existing-users/README.md) to activate Gamma on your existing installation.

Once Gamma is running, complete the following steps:
...
4. **[Create your first policy](../authorization-management/get-started/create-your-first-policy.md)** — Write a GAPL policy and apply it to a cataloged resource.

If you are evaluating Gravitee Gamma for your organization, consult the following resources:

- **[API Management overview](../api-management/get-started/api-management-overview.md).** Understand how API proxies, security plans, and the API Gateway govern REST, GraphQL, and gRPC traffic.
...
- **[Edge Management overview](../edge-management/get-started/edge-management-overview.md).** Understand how the Edge Daemon governs AI traffic on employee devices and surfaces shadow AI usage.
