...
Every component is published on `localhost` on its own port. On a single host, `localhost:8084`, `localhost:8086`, and `localhost:8083` are the **same site**, so the login session cookie is sent across ports and the consoles log in. For the detail, see [Why this works on one host](#why-this-works-on-one-host).

This guide uses the `4.12.0` images, which include the Agent Management module. For more information, see [Add your license key](#enterprise-edition-only-add-your-license-key).
...
1. [#create-the-networks](#create-the-networks "mention")
2. [#run-mongodb-and-elasticsearch](#run-mongodb-and-elasticsearch "mention")
3. [#enterprise-edition-only-add-your-license-key](#enterprise-edition-only-add-your-license-key "mention")
4. [#run-the-management-api-and-gateway](#run-the-management-api-and-gateway "mention")
5. [#run-the-consoles](#run-the-consoles "mention")
...
2. In the [Run the Management API and Gateway](#run-the-management-api-and-gateway) step, keep the `--volume` line that mounts `license.key`. For the community edition, remove that line.
...
<!-- TODO: Screenshot of the Gamma console sign-in page at localhost:8086 -->
