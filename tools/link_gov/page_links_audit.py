        ...
        for i, line in enumerate(lines, 1):
            for m in MD_LINK.finditer(line):
                ...
                if _is_absolute_root_docs(url_raw):
                    ...
                    abs_product, abs_version, abs_pv_rel = (
                        split  # e.g. ("apim","4.8","getting-started/README.md")
                    )
                    repo_rel = f"docs/{abs_product}/{abs_version}/{abs_pv_rel}"
                    exists = (DOCS_ROOT / abs_product / abs_version / abs_pv_rel).exists()

                    # Cross-PV vs same-PV
                    cur_pv_key = _pv_key_from_repo_path(src)  # pv of the *source* page
                    tgt_pv_key = f"{abs_product}/{abs_version}"  # pv of the *target* link

                    if cur_pv_key == tgt_pv_key:
                        # Same-PV absolute link: should be relative; check SUMMARY membership in current PV
                        in_summary = False
                        if cur_pv_key and cur_pv_key in pv_index:
                            in_summary = abs_pv_rel in pv_index[cur_pv_key]
                        ...
                        # Cross-PV absolute link: classify by target file existence
                        out.append(
                            Finding(
                                src.as_posix(),
                                i,
...
def _apply_fixes(findings: list[Finding], dry_run: bool = True) -> tuple[int, int]:
    ...
    for f in findings:
        if f.reason in {"needs_reformat_absolute_in_pv", "relative_exists_not_in_summary"}:
